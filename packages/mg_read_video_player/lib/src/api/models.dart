...
///
/// Responsibilities:
/// - Describe host-owned content, episodes, durable progress and visible state.
...
/// A titled video and its ordered episodes.
@immutable
final class VideoContent {
  ...
  VideoContent({
    ...
    required Iterable<VideoEpisode> episodes,
       ...
       assert(title != ''),
       episodes = List<VideoEpisode>.unmodifiable(episodes);
...
final class VideoPlaybackBackendState {
  ...
  /// Whether the first real video frame has reached the surface.
...
final class VideoPlayerSnapshot {
  ...
  /// Ordered episodes for the selection sheet.
  final List<VideoEpisode> episodes;
