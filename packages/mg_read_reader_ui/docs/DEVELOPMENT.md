...
- 已查询章节及其状态合并缓存在当前阅读器会话中；重复打开目录只补查尚未覆盖的章节。只有章节
  真实打开、下载结果变化或用户明确重试时才强制刷新目标章节，切书、能力重绑或阅读器 dispose
  会清空该会话缓存与在途记录。
- capability 抛出的异常归一化为局部可恢复失败，不阻塞当前已加载正文。

### 4.4 全局章节缓存 capability

`ReaderExtensions.chapterCacheCapability` 是可选的宿主任务入口。阅读器只提交
`ReaderChapterCacheRequest` 中的章节数、并发数和延迟；章节数由 UI 限制在 `0..全书章节数`，
并发默认 `1`，延迟默认 `3 秒`。具体章节选择、网络请求、持久写入、失败计数、跨路由进度浮条和
取消均由宿主负责。未注册 capability 时隐藏右上角更多入口；关闭阅读器不得取消已经交给宿主的任务。

宿主必须再次校验请求边界。同一全局任务被替换或关闭后停止调度后续章节；已进入网关的单个 Future
可以自然结束，但过期结果不能更新新任务。已持久缓存章节直接计入进度并连续跳过，不消费请求延迟；
只有实际下载/持久写入或请求失败后，工作线程才在下一章前等待所选延迟。阅读器不接收路径、HTTP
响应、数据库实体或下载句柄。
...
- 文本正文仅当前章与下一章；横向分页结果仅当前章和一个已完整预排的相邻下一章，并共享有界
  layout LRU。
- 横向模式当前章完整分页后，若下一章正文已就绪，只在 Flutter idle task 中按 8 段一批渐进预排整章；
  完整结果进入有界 layout LRU，最多主动准备一个 next。预排完成后，下一章第一页直接作为当前
  PageView 的右侧连续页片；跨过该页片且滚动完全停止后才提交章节状态并将控制器无感归一到新章
  第一页。纵向模式不预排布局。
...
- `ReaderObserver.onChapterPerformance` 只发送相邻预排和章节转换的 started/success/error/cancelled、
  稳定计数与耗时；宿主可映射到 `reader.chapter.performance` owner span，事件不得含章节标识或正文。
