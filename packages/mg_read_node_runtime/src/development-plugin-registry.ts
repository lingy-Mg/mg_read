...
export class DevelopmentPluginRegistry {
  ...
  async initialize(): Promise<void> {
    ...
    if (this.#npmCli !== undefined) {
      this.#monitor = new DevelopmentPluginMonitor({
        ...
        onBuildFailed: (projectRoot) => this.#queue(async () => {
          this.#events({
  ...
  #identity(projectRoot: string): { readonly pluginId?: string } {
    for (const [pluginId, candidate] of this.#candidates) {
      if (candidate.projectRoot === projectRoot) return { pluginId };
    }
    for (const [pluginId, development] of this.#loaded) {
      if (development.projectRoot === projectRoot) return { pluginId };
    }
    return {};
  }
  ...
  async #reload(projectRoot: string): Promise<void> {
    ...
    try {
      ...
      this.#events({
        code: "development_plugin_activation_failed",
        outcome: "error",
        ...(descriptor === undefined ? this.#identity(projectRoot) : { pluginId: descriptor.id }),
      });
    }
  }
