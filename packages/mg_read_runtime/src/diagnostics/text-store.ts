...
type TextRecord = Readonly<Record<string, unknown>>;
...
function baseRecord(recordType: string): TextRecord {
  return Object.freeze({ recordType, textFormatVersion });
}

function runStartRecord(run: RunRecord): TextRecord {
  return Object.freeze({
...
function sessionStartRecord(record: SessionRecord): TextRecord {
  return Object.freeze({
...
function attachmentRecord(record: AttachmentRecord): TextRecord {
  return Object.freeze({
...
function encodeRecord(record: TextRecord): string {
  return JSON.stringify(record);
}

function decodeSessionStart(record: TextRecord): SessionRecord {
...
function decodeEvent(value: unknown): RuntimeDiagnosticEvent {
...
function decodeAttachment(value: unknown): RuntimeDiagnosticAttachmentDescriptor {
...
function failedExpiredMemoryDescriptor(
...
function freezeSession(session: RuntimeDiagnosticSession): RuntimeDiagnosticSession {
...
function severityIndex(value: RuntimeDiagnosticSeverity): number {
  const index = severityOrder.indexOf(value);
  if (index < 0) throw new TypeError("Unknown Runtime diagnostic severity.");
  return index;
}

function decodeSessionState(value: string): RuntimeDiagnosticSessionState {
...
function decodeStringSet(value: unknown): ReadonlySet<string> {
...
function readString(record: TextRecord, key: string): string {
  const value = record[key];
  if (typeof value !== "string") throw new TypeError(`Diagnostic ${key} must be a string.`);
  return value;
}

function readInteger(record: TextRecord, key: string): number {
  const value = record[key];
...
async function removeLegacyDiagnostics(diagnosticsRoot: string): Promise<void> {
  await Promise.all([
    "index.sqlite",
    "index.sqlite-wal",
    "index.sqlite-shm",
