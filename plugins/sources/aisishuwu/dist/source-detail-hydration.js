 ...
 */
export async function hydrateDetailsInOrder(items, options) {
    ...
    const hydrated = new Array(items.length);
    let nextIndex = 0;
    const worker = async () => {
    ...
    const completion = Promise.all(Array.from({ length: Math.min(options.maximumConcurrency, items.length) }, () => worker()));
    ...
    return Object.freeze(items.map((item, index) => hydrated[index] ?? item));
}
