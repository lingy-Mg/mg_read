...
数据源插件。它按公开的 Plugin API v1 实现首页重磅推荐幻灯片、分类、排行、搜索、详情、目录和正文；网站 HTML、URL
...
直接读取构建后的工作区 `dist/`，下一次数据源调用会触发 development Runtime 按指纹重载，不需要先
...
数据源调用失败。主程序未来只能经 Runtime 的强类型接口统计或清理该目录，不能取得路径或文件句柄。
