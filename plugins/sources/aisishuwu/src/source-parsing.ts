...
export function mergeDiscoverySummary(
...
export function discoveryMetric(content: ContentSummary): {
...
export function requiredText(value: string | undefined): string {
...
export function novelIdFromUrl(url: URL): string | null {
  return /^\/novel\/(\d+)\.html$/u.exec(url.pathname)?.[1] ?? null;
}

export function decodeNovelId(id: string): string {
  const match = /^novel:(\d+)$/u.exec(id);
  if (match?.[1] === undefined) throw new Error('Novel ID is invalid.');
  return match[1];
}

export function decodePageCursor(cursor: string | null, scope: string): number {
...
export function encodePageCursor(scope: string, value: number): string {
  return `${scope}:${value}`;
}

export function boundedPageSize(value: number): number {
...
export function parseDetailStats($info: cheerio.Cheerio<Element>): {
...
export function parseTags($: cheerio.CheerioAPI): readonly string[] {
...
export function parseCatalogTotalCount($: cheerio.CheerioAPI): number | null {
...
export function findNextCatalogPage(
  $: cheerio.CheerioAPI,
  catalogUrl: URL,
  novelId: string,
  currentPage: number,
...
export function publicHttpUrl(value: string, base: URL): string | null {
  try {
...
export function publicCoverUrl(value: string | undefined, base: URL): string | null {
...
export function compactSourceText(value: string): string {
