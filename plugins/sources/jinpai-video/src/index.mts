 ...
 * 金牌影院会话 HTTP 数据源。
 *
 * 职责：WebView 只建立站点无感验证会话；分类、搜索、详情、选集与播放页均通过同一宿主 Profile 的 HTTP 会话读取。
 * 生命周期：activate 保存公开上下文；首次访问串行打开唯一可见 WebView，验证后的 Cookie 与 UA 仅由宿主读取。
 * IO：受保护 HTML 只通过 browser.sessionV1 transport:http；封面与 HLS 只登记给 resource.proxy。
 * 安全：插件不读取、导出、记录或重放 Cookie/令牌，也不设置 User-Agent；验证页会显示 WebView 并返回可读错误。
 */
...
export async function discover(request: { readonly target: string | null; readonly cursor: string | null; readonly collectionId: string | null; readonly pageSize: number }) {
  ...
  return frozen({ kind: 'document' as const, document: { components: [{ type: 'section', id: `${collectionId}:section`, title: category[1], subtitle: null, icon: 'video', children: [{ type: 'contentCollection', id: collectionId, layout: 'coverGrid', items, continuation }] }] } });
}
...
export async function getContent(request: { readonly id: string; readonly chapterId: string }) {
  ...
  return frozen({ chapterId: request.chapterId, contentKind: 'video' as const, title: null, updatedAt: null, text: null, pages: [], media: { url: requireContext().resource.proxy({ kind: 'hls', url: upstream, headers }), resourceType: 'hls' as const, resourcePolicy: 'sessionOnly' as const, expiresAt: null, mimeType: 'application/vnd.apple.mpegurl', headers } });
}

async function fetchSessionHtml(url: string): Promise<string> {
  await ensureBrowserSession();
  const raw = await requireContext().browser.sessionV1.request({ version: 1, sessionKey: 'jinpai-webview', url, method: 'GET', headers: { Accept: 'text/html,application/xhtml+xml' }, body: null, interaction: 'silent', presentation: 'hidden', transport: 'http', timeoutMs: 30_000, maxResponseBytes: 2 * 1024 * 1024 }) as PluginJsonValue;
...
function ensureBrowserSession(): Promise<void> {
  if (sessionBootstrap !== undefined) return sessionBootstrap;
  const candidate = withPage(async (page) => requireAccessible(page, `${base}/`));
  sessionBootstrap = candidate.catch((error: unknown) => { sessionBootstrap = undefined; throw error; });
...
function readListingsHtml(html: string): Listing[] {
...
function readDetailHtml(html: string, id: string): Detail {
...
function readEpisodesHtml(html: string, id: string): Episode[] {
...
function readM3u8Html(html: string): string {
...
function listingTitle(attributes: string, inner: string): string {
...
function htmlText(value: string): string { return clean(value.replace(/<script\b[\s\S]*?<\/script>|<style\b[\s\S]*?<\/style>|<[^>]+>/giu, ' ').replace(/&(nbsp|amp|quot|#39);/giu, (_all, entity: string) => ({ nbsp: ' ', amp: '&', quot: '"', '#39': "'" })[entity.toLowerCase()] ?? ' ')); }
function htmlAttribute(value: string, name: string): string { return clean(new RegExp(`\\b${escapeRegex(name)}\\s*=\\s*(["'])(.*?)\\1`, 'isu').exec(value)?.[2] ?? ''); }
...
function isVerificationText(value: string): boolean { return /安全验证|浏览器安全检查|recaptcha/iu.test(value); }
