...
var is = (next2, token, options, context2, compileToken2) => {
...
var maximumEntryBytes = 2 * 1024 * 1024;
var cacheFilePattern = /^[a-f0-9]{64}\.json$/u;
var BoundedTextCache = class {
  #root;
...
${url.toString()}`, "utf8").digest("hex");
  ...
  async #readOrFetch(key, policy, fetcher) {
    ...
    if (cached !== void 0 && Date.now() - cached.storedAtMs <= policy.staleAfterMs) {
      void this.#touch(key);
      return cached.body;
    }
  ...
  async #read(key) {
    try {
      ...
      if (typeof value.body !== "string" || !Number.isSafeInteger(value.storedAtMs) || Number(value.storedAtMs) < 0) {
        return void 0;
      }
  ...
  #entryPath(key) {
    if (this.#root === void 0 || !/^[a-f0-9]{64}$/u.test(key)) {
...
// src/source.ts
var siteOrigin = "https://www.4p5mha.work";
var listingPolicy = Object.freeze({
  namespace: "listing",
  staleAfterMs: 10 * 60 * 1e3,
  serveStaleWhileRevalidate: true
});
var detailPolicy = Object.freeze({
  namespace: "detail",
  staleAfterMs: 60 * 60 * 1e3,
...
var P5HanmanSource = class {
  ...
  context;
  #cache;
  #allowedSiteOrigins = /* @__PURE__ */ new Set([siteOrigin]);
  async search(query) {
    ...
    return this.#parseCards(html3, url, "unknown", "漫画", 1).items;
  }
  ...
  async getDetail(id) {
    ...
    const parsed = this.#parseDetail(html3, bookId, url);
    return Object.freeze({
      ...parsed.summary,
  ...
  async getChapters(id) {
    ...
    const chapters = this.#parseDetail(html3, bookId, url).chapters;
  ...
  #parseCards(html3, baseUrl, status, category, currentPage) {
    const $ = load(html3);
    const seen = /* @__PURE__ */ new Set();
    const items = [];
    $(".mh-item,.mh-itme-top").each((_, element) => {
      const root2 = $(element);
      const links = root2.find('a[href*="/book/"]');
      let bookId = null;
      ...
      const title = clean(root2.find(".title").first().text()) ?? clean(root2.find("a[title]").first().attr("title"));
      if (bookId === null || title === null || seen.has(bookId)) return;
      seen.add(bookId);
      const coverRaw = root2.find(".mh-cover[data-original],.lazy[data-original]").first().attr("data-original");
      const cover = coverRaw === void 0 ? null : readImageUrl(new URL(coverRaw, baseUrl).toString(), "cover");
      const author = clean(root2.find(".author").first().text());
      const latest = clean(root2.find(".chapter").first().text());
      items.push(
        summary({
          bookId,
          title,
          author,
          coverUrl: cover === null ? null : this.#proxyImage(cover, baseUrl, "cover"),
          description: null,
          status,
          chapterCount: null,
    ...
    const hasNext = $('.pagination a[href*="page="]').toArray().some((element) => {
  ...
  #parseDetail(html3, bookId, url) {
    ...
    const authorTip = info.find(".tip").filter((_, element) => /作者/u.test($(element).text())).first();
    const author = clean(authorTip.find("a").first().text()) ?? clean(authorTip.find("span").eq(1).text()) ?? clean(/作者[：:]?\s*([^\s]+)/u.exec(authorTip.text())?.[1]);
    const statusTip = info.find(".tip").filter((_, element) => /(?:状态|连载|完结)/u.test($(element).text())).first();
    const status = parseStatus(clean(statusTip.text()));
    const description = clean(info.find(".content").first().text());
    const categories2 = Object.freeze(
      info.find(".ticai a").toArray().flatMap((element) => clean($(element).text()) ?? [])
    );
    const coverRaw = $(".banner_detail .cover img[data-original]").first().attr("data-original");
    const cover = coverRaw === void 0 ? null : readImageUrl(new URL(coverRaw, url).toString(), "cover");
    const seen = /* @__PURE__ */ new Set();
    const chapters = [];
    ...
    const currentSummary = summary({
      ...
      title,
      author,
      coverUrl: cover === null ? null : this.#proxyImage(cover, url, "cover"),
      description,
      status,
      chapterCount: chapters.length,
      latest: latestChapter === void 0 ? null : Object.freeze({
        id: latestChapter.id,
        title: latestChapter.title,
        url: latestChapter.url,
        updatedAt: null
      }),
      categories: categories2.length === 0 ? ["漫画"] : categories2
    });
    return Object.freeze({
  ...
  #proxyImage(url, referer, purpose) {
  ...
  #isAllowedReferer(url) {
    return this.#allowedSiteOrigins.has(url.origin);
  }
  #readAllowedReferer(value, purpose) {
    ...
    if (purpose === "cover" && !/^\/(?:$|update\/?$|rank\/?$|booklist\/?$|search\/?$|book\/\d+\/?$)/u.test(
      referer.pathname
    )) {
      return null;
    }
...
function parseBookId(value) {
  ...
  if (bookId === void 0) throw new Error("Content ID is invalid.");
  return bookId;
}
function contentChapterId(bookId, chapterId) {
...
function readImageUrl(value, purpose) {
  const url = readHttpsUrl(value);
  if (url === null) return null;
  if (purpose === "page") {
    return url.hostname === "cfpic.se8manhua.club" ? url : null;
  }
  return url.hostname === "cover.aabamh.com" || url.hostname === "stpic.se8manhua.club" ? url : null;
}
...
function pageId(url) {
  return `page:${createHash2("sha256").update(url.toString()).digest("hex").slice(0, 24)}`;
}
...
async function discover(request) {
  return invoke("discover", async () => {
    ...
    if (categoryId === void 0 || !categories.some(([id]) => id === categoryId)) {
    ...
    const title = categories.find(([id]) => id === categoryId)?.[1] ?? "漫画";
    return Object.freeze({
      kind: "document",
      document: {
              ...
              categories: Object.freeze(
                categories.map(
                  ([id, title]) => Object.freeze({
                    ...
                    count: null,
                    url: null,
                    icon: id === "latest" ? "newRelease" : id === "popular" ? "hot" : "completed"
                  })
                )
              )
            })
...
function decodeDiscoveryCursor(value, categoryId) {
  ...
  if (match?.[1] !== categoryId || !Number.isSafeInteger(page) || page < 1 || !Number.isSafeInteger(offset) || offset < 0) {
