...
export class P5HanmanSource {
    context;
    #cache;
    #allowedSiteOrigins = new Set([siteOrigin]);
    constructor(context) {
    ...
    async discover(categoryId, page) {
        ...
        const html = await this.#cache.getOrFetchText(url, listingPolicy, async () => (await this.#fetchHtml(url)).body);
        return this.#parseCards(html, url, categoryId === 'completed' ? 'completed' : 'unknown', category[1], page);
    }
    async getDetail(id) {
        const bookId = parseBookId(id);
        const url = bookUrl(bookId);
        const html = await this.#cache.getOrFetchText(url, detailPolicy, async () => (await this.#fetchHtml(url)).body);
    ...
    async getContent(id, chapterId) {
        ...
        $('.comicpage img.lazy[data-original]').each((_, element) => {
            ...
            if (image === null || seen.has(image.toString()))
                return;
        ...
        return Object.freeze({
            ...
            updatedAt: null,
            text: null,
            pages: Object.freeze(images.map((image, index) => Object.freeze({
                id: pageId(image),
                index,
    ...
    #parseCards(html, baseUrl, status, category, currentPage) {
    ...
    #parseDetail(html, bookId, url) {
        ...
        const authorTip = info
            ...
            .first();
        const author = clean(authorTip.find('a').first().text()) ??
        ...
        const categories = Object.freeze(info
            .find('.ticai a')
            .toArray()
        ...
        $('.detail-list-select a[href*="/chapter/"]').each((_, element) => {
            ...
            if (chapterId === null || seen.has(chapterId))
                return;
            seen.add(chapterId);
        ...
        const currentSummary = summary({
            ...
            latest: latestChapter === undefined
                ? null
                : Object.freeze({
                    id: latestChapter.id,
    ...
    #readAllowedReferer(value, purpose) {
        ...
        if (referer === null || !this.#isAllowedReferer(referer))
            return null;
        if (purpose === 'page' && !/^\/chapter\/\d+\/?$/u.test(referer.pathname)) {
            return null;
        }
        if (purpose === 'cover' &&
            !/^\/(?:$|update\/?$|rank\/?$|booklist\/?$|search\/?$|book\/\d+\/?$)/u.test(referer.pathname)) {
            return null;
        }
        return referer;
    }
...
function listingUrl(categoryId, page) {
    if (categoryId === 'popular') {
        const url = new URL('/rank', siteOrigin);
        if (page > 1)
    ...
    if (['all', 'ongoing', 'korea', 'korea-ongoing', 'korea-completed', 'other', 'other-ongoing', 'other-completed'].includes(categoryId)) {
        ...
        url.searchParams.set('cate', '全部');
        url.searchParams.set('area', categoryId.startsWith('korea') ? '1' : categoryId.startsWith('other') ? '2' : '-1');
    ...
    if (categoryId !== 'latest')
        throw new Error('Unknown category.');
    if (page === 1)
        return new URL('/update', siteOrigin);
    const url = new URL('/booklist', siteOrigin);
    url.searchParams.set('page', String(page));
    return url;
}
