...
export async function discover(request) {
    if (request.target === null) {
        ...
        return frozen({ kind: 'document', document: { components: [{ type: 'section', id: 'huya-categories', title: '虎牙直播', subtitle: '按游戏与频道浏览', icon: 'video', children: [{ type: 'categoryCollection', id: 'huya-category-list', layout: 'chips', categories: categories.map(([id, title]) => ({ id, title, target: `category:${id}`, count: null, url: null, icon: 'video' })) }] }] } });
    }
    ...
    const response = await requireContext().http.fetch(`${listApi}?iGid=${encodeURIComponent(id)}&iPageNo=${page}&iPageSize=${Math.max(limit, 30)}`, { headers });
    if (!response.ok)
        throw new Error('Source request failed.');
    const json = await response.json();
    if (!isObject(json))
        throw new Error('Source response is invalid.');
    const values = pickList(json).map(roomSummary).filter(notNull).slice(0, limit), collectionId = `huya:${id}`;
...
export async function getDetail(request) { const roomId = contentId(request.id), data = await loadDetail(roomId), info = roomInfo(data, roomId), item = summary(roomId, info.title, info.author, info.cover, info.category); return frozen({ ...item, description: `房间号：${roomId}${info.author ? `\n主播：${info.author}` : ''}\n不要相信视频中的任何广告。`, aliases: [], catalogUrl: `${web}${encodeURIComponent(roomId)}` }); }
export async function getChapters(request) { const roomId = contentId(request.id), data = await loadDetail(roomId), streams = collectStreams(data), items = streams.map((stream, index) => chapter(roomId, stream, index)); return frozen({ items, groups: items.length === 0 ? [] : [frozen({ id: `group:${encodeKey(roomId)}:live`, title: '直播线路', order: 0, episodes: items })] }); }
export async function getContent(request) { const roomId = contentId(request.id), key = chapterKey(request.chapterId, roomId), streams = collectStreams(await loadDetail(roomId)), stream = streams.find((value, index) => streamKey(value, index) === key); if (stream === undefined)
    throw new Error('Chapter ID is invalid.'); const upstream = streamUrl(stream); if (!safeUrl(upstream))
...
async function loadDetail(roomId) { const response = await requireContext().http.fetch(`${detailApi}?rid=${encodeURIComponent(roomId)}`, { headers }); if (!response.ok)
    throw new Error('Source request failed.'); const raw = await response.text(); const value = parsePayload(raw); if (!isObject(value))
...
function roomInfo(data, roomId) { const info = object(first(data.roomInfo, data.profileInfo, data.liveData, data.room)); return { title: text(first(info.vod_name, info.roomName, info.sIntroduction, info.introduction, info.title)) || `虎牙房间 ${roomId}`, author: text(first(info.vod_actor, info.sNick, info.nick, info.nickName, info.profileNick)), cover: text(first(info.vod_pic, info.sScreenshot, info.screenshot, info.avatar180, info.avatar)), category: text(first(info.vod_remarks, info.type_name, info.gameName)) }; }
function collectStreams(data) { const groups = records(first(data.data, data.urls, data.lines)); const result = []; for (const group of groups) {
    const entries = records(first(group.urls, group.urlList, group.lines, group.items));
    for (const entry of entries.length > 0 ? entries : [group])
        if (streamUrl(entry) !== '' && !result.some(old => streamUrl(old) === streamUrl(entry)))
            result.push(entry);
} if (result.length === 0 && streamUrl(data) !== '')
    result.push(data); return result; }
function pickList(root) { for (const value of [root.vList, root.data, object(root.data).vList, object(root.data).list, object(root.data).datas, root.list, root.datas]) {
    const list = records(value);
    if (list.length > 0)
        return list;
} return []; }
function roomSummary(item) { const id = text(first(item.lProfileRoom, item.profileRoom, item.roomId, item.room_id, item.uid, item.privateHost, item.yyid)); if (id === '')
    return null; return summary(id, text(first(item.sIntroduction, item.introduction, item.roomName, item.title)) || `虎牙房间 ${id}`, text(first(item.sNick, item.nick, item.nickName, item.profileNick, item.nickname)), text(first(item.sScreenshot, item.screenshot, item.img, item.sAvatar180, item.avatar180, item.avatar, item.roomPic)), text(first(item.sGameFullName, item.gameFullName, item.gameName, item.gamename))); }
function summary(id, title, author, cover, category) { const key = encodeKey(id); return frozen({ id: `live:${key}`, title, contentKind: 'video', coverOrientation: 'landscape', author: author || null, url: `${web}${encodeURIComponent(id)}`, coverUrl: proxyImage(cover), description: `房间号：${id}${category ? `\n分类：${category}` : ''}`, language: 'zh-CN', status: 'ongoing', access: 'free', wordCount: null, chapterCount: 1, publishedAt: null, updatedAt: null, latestChapter: { id: `live:${key}:main`, title: category || '直播', url: null, updatedAt: null }, categories: category ? [category] : ['直播'], tags: [], attributes: [] }); }
function chapter(roomId, stream, index) { return frozen({ id: `live:${encodeKey(roomId)}:${streamKey(stream, index)}`, title: text(first(stream.displayName, stream.name, stream.cdnName, stream.title)) || `线路${index + 1}`, order: index, url: null, volumeTitle: '直播线路', wordCount: null, updatedAt: null, isLocked: null, attributes: [] }); }
function streamUrl(value) { return text(first(value.url, value.playUrl, value.m3u8, value.flv, value.play_url)); }
function streamKey(value, index) { return createHash('sha256').update(streamUrl(value) || String(index)).digest('hex').slice(0, 24); }
...
function chapterKey(id, roomId) { const prefix = `live:${encodeKey(roomId)}:`; if (!id.startsWith(prefix) || id.length === prefix.length)
