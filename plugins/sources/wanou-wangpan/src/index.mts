 ...
 * 玩偶网盘视频数据源。
 *
 * 职责：读取 wogg.net 的影视索引，识别百度/夸克/优汐分享，并在宿主 WebView
 * 已完成用户登录后读取分享目录与可播放地址。
 * 生命周期：activate 仅保存公开上下文；每个插件只使用宿主持有的一张 WebView，
 * 所有导航按 FIFO 串行，绝不读取或持久化 Cookie、密码或访问令牌。
 * IO：站点 HTML 走 ctx.http；网盘的有凭据 API 走当前 WebView 页面；封面和媒体
 * 始终经 ctx.resource.proxy。登录是用户在可见 WebView 中主动完成的交互。
 * 稳定标识：内容使用玩偶详情路径；章节使用 provider + 分享 ID + 文件 ID 的
 * base64url 编码不透明值，绝不把页序号或临时下载 URL 当作身份。
 */
...
type Provider = 'baidu' | 'quark' | 'uc';
type Share = { readonly provider: Provider; readonly url: string; readonly id: string; readonly passcode: string };
type CloudFile = { readonly id: string; readonly name: string; readonly path: string; readonly size: number; readonly token: string | null };
type Chapter = { readonly share: Share; readonly file: CloudFile };
type Listing = { readonly id: string; readonly title: string; readonly cover: string; readonly latest: string; readonly url: string };
...
export async function discover(request: { target: string | null; cursor: string | null; collectionId: string | null; pageSize: number }) {
  ...
  if (request.target.startsWith('login:')) return openLogin(request.target);
...
export async function getChapters(request: { id: string }) {
  ...
  const groups: Array<{ readonly id: string; readonly title: string; readonly order: number; readonly episodes: readonly object[] }> = [];
  const items: object[] = [];
  for (const [index, share] of shares.entries()) {
    let files: readonly CloudFile[] = [];
    try { files = await listShareFiles(share); }
    catch (error) {
...
export async function getContent(request: { id: string; chapterId: string }) {
  ...
  const resourceType = /\.m3u8(?:$|[?#])/iu.test(resolved.url) ? 'hls' as const : 'video' as const;
  const headers = { Referer: resolved.referer, 'User-Agent': userAgent };
  return frozen({
    chapterId: request.chapterId, contentKind: 'video' as const, title: chapter.file.name, updatedAt: null, text: null, pages: [],
    media: { url: requireContext().resource.proxy({ kind: resourceType, url: resolved.url, headers }), resourceType,
      resourcePolicy: 'sessionOnly' as const, expiresAt: null, mimeType: resourceType === 'hls' ? 'application/vnd.apple.mpegurl' : 'video/mp4', headers },
  });
}

async function openLogin(target: string) {
...
async function listShareFiles(share: Share): Promise<readonly CloudFile[]> {
  if (share.provider === 'quark' || share.provider === 'uc') return listUcLikeShare(share);
  return listBaiduShare(share);
}

async function listUcLikeShare(share: Share): Promise<readonly CloudFile[]> {
  ...
  return withPage(async (page) => {
    ...
    if (stoken === '') throw new Error('login_or_share_password_required');
    const queue = [{ id: '0', path: '' }]; const files: CloudFile[] = []; const visited = new Set<string>();
    while (queue.length > 0 && visited.size < 30) {
      const folder = queue.shift(); if (folder === undefined || visited.has(folder.id)) continue; visited.add(folder.id);
      ...
      for (const value of list) {
        ...
        const id = firstString(value.fid, value.file_id); const name = firstString(value.file_name, value.name);
        if (id === '' || name === '') continue;
        const path = `${folder.path}${name}`; const dir = value.dir === true || value.dir === 1 || value.file === false || value.file_type === 'folder';
        if (dir) { queue.push({ id, path: `${path}/` }); continue; }
        if (isVideo(name, value)) files.push({ id, name, path, size: number(value.size, value.file_size), token: nullable(firstString(value.share_fid_token)) });
      }
    }
    if (files.length === 0) throw new Error('no_playable_files_or_login_required');
    return files.sort((a, b) => a.path.localeCompare(b.path, 'zh-CN', { numeric: true }));
  });
}

async function listBaiduShare(share: Share): Promise<readonly CloudFile[]> {
  return withPage(async (page) => {
    await page.navigate(share.url, { timeoutMs: 45_000 });
    ...
    const files = list.flatMap((value) => {
      if (!isRecord(value) || number(value.isdir, value.is_dir) !== 0) return [];
      const id = firstString(value.fs_id, value.fsid); const name = firstString(value.server_filename, value.name); if (id === '' || name === '' || !isVideo(name, value)) return [];
      return [{ id, name, path: firstString(value.path, name), size: number(value.size), token: null } satisfies CloudFile];
    });
    if (files.length === 0) throw new Error('no_playable_files_or_login_required');
    return files.sort((a, b) => a.path.localeCompare(b.path, 'zh-CN', { numeric: true }));
  });
...
async function resolvePlayback(chapter: Chapter): Promise<{ readonly url: string; readonly referer: string }> {
  ...
  return withPage(async (page) => {
    ...
    const raw = await page.executeJavaScript<PluginJsonValue>(`(async()=>{
    ...
    if (url === '') throw new Error('播放地址未由网盘页面提供。请确认已登录并在官方分享页有播放权限。');
...
function collectionResponse(id: string, title: string, request: { cursor: string | null; collectionId: string | null; pageSize: number; target: string | null }, values: readonly Listing[], page: number) {
  ...
  if (request.collectionId !== null) { if (request.collectionId !== id) throw new Error('Discovery collection is invalid.'); return frozen({ kind: 'append' as const, collectionId: id, items, continuation }); }
  return frozen({ kind: 'document' as const, document: { components: [{ type: 'section', id: `${id}:section`, title, subtitle: null, icon: 'video', children: [{ type: 'contentCollection', id, layout: 'coverGrid', items, continuation }] }] } });
}
...
function listingFromHtml(html: string): Listing[] {
  ...
  const url = absolute(href); if (title === '' || url === '') return [];
...
function parseDetail(html: string, url: string): Listing {
...
function extractShares(html: string): readonly Share[] {
  const variants = [html, html.replaceAll('\\/', '/').replaceAll('&amp;', '&')]; const values: Share[] = [];
  for (const value of variants) {
    for (const match of value.matchAll(/https?:\/\/(?:pan\.quark\.cn\/s\/[A-Za-z0-9_-]+|(?:pan\.baidu\.com\/s\/[A-Za-z0-9_-]+)|(?:(?:drive|pan)\.uc\.cn\/s\/[A-Za-z0-9_-]+))(?:\?[^"'<>\s]*)?/giu)) {
      const raw = match[0]; const provider: Provider = /quark/iu.test(raw) ? 'quark' : /baidu/iu.test(raw) ? 'baidu' : 'uc';
      const id = provider === 'baidu' ? (/\/s\/1?([A-Za-z0-9_-]+)/iu.exec(raw)?.[1] ?? '') : (/\/s\/([A-Za-z0-9_-]+)/iu.exec(raw)?.[1] ?? '');
      if (id === '') continue; values.push({ provider, id, url: raw.replace(/[?#].*$/u, ''), passcode: passwordNear(value, match.index ?? 0) });
    }
  }
  return [...new Map(values.map((share) => [`${share.provider}:${share.id}`, share])).values()];
}

function summary(value: Listing) { return frozen({ id: `wanou:${value.id}`, title: value.title, contentKind: 'video' as const, coverOrientation: 'portrait' as const, author: null, url: value.url, coverUrl: value.cover === '' ? null : requireContext().resource.proxy({ kind: 'image', url: value.cover, headers: { Referer: `${site}/` } }), description: value.latest === '' ? null : value.latest, language: 'zh-CN', status: 'unknown' as const, access: 'unknown' as const, wordCount: null, chapterCount: null, publishedAt: null, updatedAt: null, latestChapter: null, categories: [], tags: [], attributes: [] }); }
function chapterItem(share: Share, file: CloudFile, order: number) { const id = encodeText(JSON.stringify({ share, file })); return frozen({ id: `wanou-chapter:${id}`, title: file.name, order, url: null, volumeTitle: providerLabel(share.provider), wordCount: null, updatedAt: null, isLocked: file.id === 'login', attributes: [] }); }
function decodeChapter(value: string): Chapter { const encoded = /^wanou-chapter:([A-Za-z0-9_-]+)$/u.exec(value)?.[1]; if (encoded === undefined) throw new Error('Chapter ID is invalid.'); try { const parsed = JSON.parse(decodeText(encoded)) as unknown; if (!isRecord(parsed) || !isRecord(parsed.share) || !isRecord(parsed.file)) throw new Error(); const provider = parsed.share.provider; if (provider !== 'baidu' && provider !== 'quark' && provider !== 'uc') throw new Error(); const url = firstString(parsed.share.url); const id = firstString(parsed.share.id); const name = firstString(parsed.file.name); const fileId = firstString(parsed.file.id); if (url === '' || id === '' || name === '' || fileId === '') throw new Error(); return { share: { provider, url, id, passcode: firstString(parsed.share.passcode) }, file: { id: fileId, name, path: firstString(parsed.file.path), size: number(parsed.file.size), token: nullable(firstString(parsed.file.token)) } }; } catch { throw new Error('Chapter ID is invalid.'); } }
function contentUrl(value: string): string { const encoded = /^wanou:([A-Za-z0-9_-]+)$/u.exec(value)?.[1]; if (encoded === undefined) throw new Error('Content ID is invalid.'); const url = safeUrl(decodeText(encoded)); if (!url.startsWith(site)) throw new Error('Content ID is invalid.'); return url; }
function categoryUrl(id: string, page: number): string { return page === 1 ? `${site}/vodshow/${id}-----------.html` : `${site}/vodshow/${id}--------${page}---.html`; }
function withPage<T>(action: (page: PluginWebViewPage) => Promise<T>): Promise<T> { const run = pageQueue.then(async () => action(await requireContext().webview.open({ visible: false, timeoutMs: 30_000 }))); pageQueue = run.then(() => undefined, () => undefined); return run; }
function providerLabel(provider: Provider): string { return provider === 'quark' ? '夸克网盘' : provider === 'uc' ? '优汐网盘' : '百度网盘'; }
function loginHint(provider: Provider): string { return `${providerLabel(provider)}（请先在发现页的“网盘登录”完成登录）`; }
function isVideo(name: string, value: Json): boolean { const extension = /\.([a-z0-9]+)(?:$|[?#])/iu.exec(name)?.[1]?.toLowerCase() ?? ''; return videoExtensions.has(extension) || /video/iu.test(firstString(value.obj_category, value.category, value.file_type)); }
function nestedArray(value: PluginJsonValue, keys: readonly string[]): readonly PluginJsonValue[] { let current: PluginJsonValue | undefined = value; for (const key of keys) { if (!isRecord(current)) return []; current = current[key]; } return Array.isArray(current) ? current : []; }
function nestedString(value: PluginJsonValue, keys: readonly string[]): string { let current: PluginJsonValue | undefined = value; for (const key of keys) { if (!isRecord(current)) return ''; current = current[key]; } return firstString(current); }
function firstArray(value: PluginJsonValue, ...keys: string[]): readonly PluginJsonValue[] { if (!isRecord(value)) return []; for (const key of keys) { const candidate = value[key]; if (Array.isArray(candidate)) return candidate; if (isRecord(candidate) && Array.isArray(candidate.list)) return candidate.list; } return []; }
function firstString(...values: unknown[]): string { for (const value of values) if (typeof value === 'string' || typeof value === 'number') { const text = String(value).trim(); if (text !== '') return text; } return ''; }
function number(...values: unknown[]): number { const value = Number(values.find((item) => item !== null && item !== undefined && item !== '') ?? 0); return Number.isFinite(value) && value >= 0 ? value : 0; }
function nullable(value: string): string | null { return value === '' ? null : value; }
function cursorPage(cursor: string | null, target: string): number { if (cursor === null) return 1; const value = Number(cursor.startsWith(`${target}:`) ? cursor.slice(target.length + 1) : ''); if (!Number.isSafeInteger(value) || value < 2 || value > 50) throw new Error('Cursor is invalid.'); return value; }
function clamp(value: number): number { return Math.max(1, Math.min(50, Math.floor(value))); }
function absolute(value: string): string { if (value === '') return ''; try { return new URL(value, site).toString(); } catch { return ''; } }
function safeUrl(value: string): string { try { const url = new URL(value); return url.protocol === 'https:' || url.protocol === 'http:' ? url.toString() : ''; } catch { return ''; } }
function attribute(value: string, pattern: RegExp): string { return pattern.exec(value)?.[1] ? decodeHtml(pattern.exec(value)?.[1] ?? '') : ''; }
function firstMatch(value: string, pattern: RegExp): string { return pattern.exec(value)?.[1] ?? ''; }
function textOf(value: string): string { return decodeHtml(value.replace(/<[^>]+>/gu, ' ').replace(/\s+/gu, ' ')); }
function decodeHtml(value: string): string { return value.replace(/&(?:amp|#38);/giu, '&').replace(/&nbsp;/giu, ' ').replace(/&quot;/giu, '"').replace(/&#39;/giu, "'"); }
function clean(value: string): string { return value.replace(/[\s\u3000\u00a0]+/gu, ' ').trim(); }
function passwordNear(value: string, index: number): string { return /(?:提取码|密码|pwd|passcode)\s*[:：=]\s*([A-Za-z0-9]{4,})/iu.exec(value.slice(Math.max(0, index - 120), index + 240))?.[1] ?? ''; }
function encodeText(value: string): string { return Buffer.from(value, 'utf8').toString('base64url'); }
function decodeText(value: string): string { return Buffer.from(value, 'base64url').toString('utf8'); }
...
function safeError(error: unknown): string { return error instanceof Error ? error.message.slice(0, 120) : 'unknown'; }
