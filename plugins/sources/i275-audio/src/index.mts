...
async function fetchPage(url: string, referer: string): Promise<BrowserPageResponse> {
...
function parseBooks(html: string) {
  const $ = load(html);
  const values = new Map<string, ReturnType<typeof summary>>();
  $('a[href^="/book/"]').each((_, node) => {
    const href = $(node).attr('href') ?? '';
    const id = /\/book\/(\d+)\.html/u.exec(href)?.[1];
    if (id === undefined || values.has(id)) return;
...
function parseChapters($: ReturnType<typeof load>, book: string) {
...
function summary(id: string, title: string, cover: string, description: string) {
  return frozen({
    id: `audio:${id}`,
    title,
    contentKind: 'audio',
...
function audioFromHtml(html: string) {
  const patterns = [
...
function chapterUrl(book: string, chapter: string) { return `${base}/play/${book}/${chapter}.html`; }
function contentId(id: string) {
  const value = /^audio:(\d+)$/u.exec(id)?.[1];
  if (value === undefined) throw new Error('Content ID is invalid.');
  return value;
}
function parseChapterId(id: string, book: string) {
  const value = new RegExp(`^audio:${book}:(\\d+)$`, 'u').exec(id)?.[1];
  if (value === undefined) throw new Error('Chapter ID is invalid.');
  return value;
}
function proxyImage(value: string) {
...
function absolute(value: string) {
  ...
  if (raw === '') return null;
  try { return new URL(raw.replaceAll('\\/', '/'), base).toString(); } catch { return null; }
}
function isAudio(url: string) { return /\.(?:mp3|m4a|aac|flac|wav|ogg|opus|ape|wma)(?:$|[?#])/iu.test(url); }
...
function cursorPage(cursor: string | null, target: string) {
  ...
  if (!Number.isSafeInteger(page) || page < 2 || page > 1000) throw new Error('Cursor is invalid.');
  return page;
}
