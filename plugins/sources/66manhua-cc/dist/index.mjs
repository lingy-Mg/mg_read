...
var ManhuaSource = class {
  ...
  async chapters(id) {
    ...
    const $ = load(await this.#html(comic));
    const items = [];
  ...
  #rankedItems($, roots, base, metricLabel) {
    ...
    roots.each((index2, element) => {
      ...
      values.push(Object.freeze({
        content,
        rank: Number.isSafeInteger(parsedRank) && parsedRank > 0 ? parsedRank : index2 + 1,
  ...
  #card($, root2, base) {
    ...
    const description = clean(root2.find(".comic__feature, .feature").first().text()) ?? (info.length > 1 ? clean(info.last().text()) : null);
    const latestChapterTitle = clean(root2.find(".cover__tag").first().text());
...
function summary(url, title, coverUrl, description, author = null, latestChapterTitle = null) {
...
function token(url) {
  return Buffer2.from(url.pathname, "utf8").toString("base64url");
}
function encodeComic(url) {
  return `comic:${token(url)}`;
}
function encodeChapter(url) {
  return `chapter:${token(url)}`;
}
...
function decodeComic(id) {
  return decode(id, "comic");
}
function decodeChapter(id) {
  return decode(id, "chapter");
}
function isSite(url) {
  return siteOrigins.has(url.origin) && url.protocol === "https:";
}
function isComic(url) {
  return isSite(url) && /^\/index\.php\/comic\/[^/?#]+\/?$/u.test(url.pathname);
}
function isChapter(url) {
  return isSite(url) && /^\/index\.php\/chapter\/\d+\/?$/u.test(url.pathname);
}
function isImage(url) {
  return url.protocol === "https:" && imageOrigins.has(url.origin) && /\.(?:jpe?g|png|webp|gif)(?:$|\?)/iu.test(url.pathname + url.search);
}
function imageMime(url) {
  const ext = /\.([a-z]+)(?:$|\?)/iu.exec(url.pathname + url.search)?.[1]?.toLowerCase();
...
function hasAccessMarker(root2) {
