...
import { RehanmanSource } from './source.js';
let source;
...
function parseCursor(value, scope = 'latest') { if (value === null)
    return 1; const match = new RegExp(`^${scope}:(\\d+)$`, 'u').exec(value); const page = Number(match?.[1]); if (!Number.isSafeInteger(page) || page < 2)
