...
export class RehanmanSource {
    ...
    async detail(id) { const entry = await this.#entry(id); return Object.freeze({ ...summary(entry), aliases: Object.freeze([]), catalogUrl: bookUrl(entry).toString() }); }
    ...
    async #entries(inputs) { const response = await this.context.http.fetch(graphQlUrl, { method: 'POST', headers: { accept: 'application/json', 'content-type': 'application/json' }, body: JSON.stringify({ query: entriesQuery, variables: { inputs: { ...inputs, adult: true, is_hide: false } } }) }); if (!response.ok)
        throw new Error('Source listing is unavailable.'); const payload = object(await response.json()); const entries = object(object(payload?.data)?.entries); const items = array(entries?.docs).flatMap((value) => { const entry = parseEntry(value); return entry === null ? [] : [summary(entry)]; }); const current = number(entries?.page); const totalPages = number(entries?.totalPages); const totalCount = number(entries?.totalDocs); if (current === null || totalPages === null || totalCount === null)
