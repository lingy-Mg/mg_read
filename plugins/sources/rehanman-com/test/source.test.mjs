...
const listingJson = JSON.stringify({ data: { entries: { docs: [entry], page: 1, totalPages: 1, totalDocs: 1 } } });

test('fixture chain emits public manga manifests and proxies only the image host', async () => {
  ...
  const discovery = await plugin.discover({ target: null, collectionId: null, cursor: null, pageSize: 20 });
  ...
  const content = await plugin.getContent({ id: item.id, chapterId: chapters.items[0].id }); assert.equal(content.text, null); assert.equal(content.pages.length, 2); assert.deepEqual(proxied.map((request) => request.kind), ['rehanman-image', 'rehanman-image']);
