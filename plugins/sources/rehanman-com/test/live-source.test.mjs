...
test('public target chain remains readable without credentials', async () => {
  await plugin.activate({ dataDir: 'unused', app: {}, plugin: {}, log: { debug() {}, info() {}, warn() {}, error() {} }, resource: { proxy(request) { assert.equal(request.kind, 'rehanman-image'); assert.equal(typeof request.url, 'string'); assert.equal(typeof request.headers?.Referer, 'string'); return request.url; } }, http: { fetch(input, init) { return fetch(input, init); } } });
  ...
  assert.equal(detail.id, id); assert.match(detail.coverUrl ?? '', /^https:\/\/img\.rehanman\.com\//u); assert.ok(chapters.items.length > 0); assert.ok(content.pages.length > 0); assert.equal(content.text, null); assert.equal(content.pages[0].resourcePolicy, undefined);
