...
test('public target chain remains readable without credentials', async () => {
  const proxied = [];
  await plugin.activate({ dataDir: 'unused', app: {}, plugin: {}, log: { debug() {}, info() {}, warn() {}, error() {} }, resource: { proxy(request) { proxied.push(request); return `http://127.0.0.1/resource/${proxied.length}`; } }, http: { fetch(input, init) { return fetch(input, init); } } });
  ...
  assert.equal(detail.id, id); assert.ok(chapters.items.length > 0); assert.ok(content.pages.length > 0); assert.equal(content.text, null); assert.ok(proxied.length > 0);
});
