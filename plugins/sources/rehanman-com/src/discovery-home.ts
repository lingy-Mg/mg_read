...
export function homeEntries(html: string) {
  return html.split(/<div\s+class="slider-product"[^>]*>/u).slice(1).flatMap(block => {
    ...
    const entries = [...block.matchAll(/<a\b([^>]*\bhref="\/webtoon\/(\d+)"[^>]*)>([\s\S]*?)<\/a>/gu)].flatMap(match => {
      ...
      if (!title || seen.has(id)) return []; seen.add(id);
      const images = [...match[3]!.matchAll(/\bsrc="([^"]+)"/gu)].map(value => decode(value[1]!));
      const thumbnail = images.flatMap(raw => {
        try { const url = new URL(raw, 'https://rehanman.com'); const value = url.pathname === '/_next/image' ? url.searchParams.get('url') : url.toString(); return value?.startsWith('https://img.rehanman.com/') ? [value] : []; } catch { return []; }
      })[0] ?? null;
...
function decode(value: string) {
  return value.replace(/<[^>]+>/gu, '').replace(/&#(x[0-9a-f]+|\d+);/giu, (_, code: string) => { const n = code[0]?.toLowerCase() === 'x' ? parseInt(code.slice(1), 16) : Number(code); return n <= 0x10ffff ? String.fromCodePoint(n) : ''; }).replaceAll('&quot;', '"').replaceAll('&#39;', "'").replaceAll('&lt;', '<').replaceAll('&gt;', '>').replaceAll('&amp;', '&').trim();
}
