...
type DiscoverTarget =
  | { readonly kind: 'hot' }
  | { readonly kind: 'rank' }
  | { readonly id: string; readonly kind: 'category' }
  | { readonly code: string; readonly fallbackCategoryId: string | null; readonly kind: 'nav' };
type DramaOptions = {
...
export async function search(request: { readonly cursor: string | null; readonly pageSize: number; readonly query: string }) {
...
export async function searchSuggestions(request: { readonly cursor: string | null; readonly pageSize: number }) {
...
export async function discover(request: { readonly collectionId: string | null; readonly cursor: string | null; readonly pageSize: number; readonly target: string | null }) {
...
export async function getContent(request: { readonly chapterId: string; readonly id: string }) {
  const id = decodeDramaId(request.id);
  ...
  if (episode === undefined) throw new Error('Episode ID is invalid.');

  let play: Json | null = null;
  try {
...
async function rootDiscovery(pageSize: number) {
...
function discoveryCategories(navs: readonly Json[], configured: readonly Json[]) {
  const result: object[] = [];
  const seen = new Set<string>();
  const add = (title: string, target: string, icon: 'hot' | 'ranking' | 'video') => {
...
async function discoverItems(target: DiscoverTarget, page: number, pageSize: number) {
...
async function fetchNavFilters(code: string): Promise<Json | null> {
  try {
    const data = await apiData('/drama/navFilter', { code });
    const first = records(data.list)[0];
    if (first !== undefined && isObject(first.filter)) return first.filter;
  } catch (error) {
    if (!(error instanceof ApiFailure)) throw error;
  }
  try {
...
async function fetchDramaList(page: number, options: DramaOptions) {
  const request: Json = { page: String(page), page_size: String(options.pageSize ?? 8) };
...
async function fetchDramaPlay(id: string, sequence: string) {
  return apiData('/drama/play', { id, seq: sequence });
}

async function apiData(path: string, data: Json): Promise<Json> {
  const response = await postApi(path, data, true);
  return isObject(response.data) ? response.data : {};
}

async function postApi(path: string, data: Json, authenticated: boolean): Promise<Json> {
...
async function postAcrossHosts(path: string, data: Json, token: string): Promise<Json> {
...
async function postOnHost(host: string, path: string, data: Json, token: string): Promise<Json> {
  ...
  const value: unknown = JSON.parse(gunzipSync(decrypted).toString('utf8'));
  if (!isObject(value)) throw new Error('Source response is invalid.');
...
class ApiFailure extends Error {
  constructor(readonly authExpired: boolean) {
    super('Source API rejected the request.');
    this.name = 'ApiFailure';
  }
}

function apiFailure(value: Json) {
...
function dramaSummary(item: Json) {
  ...
  return frozen({
    ...
    access: pay.kind === 'free' ? 'free' as const : 'mixed' as const,
    wordCount: null,
...
function episodeSummary(dramaId: string, episode: Json, index: number) {
...
function playableFrom(play: Json | null, detail: Json, episode: Json) {
  const direct = play === null ? '' : text(play.m3u8);
  if (safeMediaUrl(direct)) return { url: direct, kind: 'hls' as const };
  if (play !== null) {
    ...
    if (safeMediaUrl(preview)) return { url: preview, kind: 'hls' as const };
  }
...
function payment(item: Json) {
  ...
  if (type === 'vip' || truthy(item.vip) || truthy(item.is_vip)) return { kind: 'vip' as const, label: 'VIP', price: amount };
  if (type === 'coin' || type === 'money' || type === 'points' || (amount !== null && Number(amount) > 0)) {
    return { kind: 'coin' as const, label: '金币', price: amount };
  }
  return { kind: 'free' as const, label: null, price: null };
}

function episodeLocked(item: Json, kind: 'coin' | 'free' | 'vip') {
  if (kind === 'free') return false;
  const bought = knownBoolean(item.is_buy ?? item.bought);
  return bought === null ? true : !bought;
}

function proxyCover(value: unknown) {
...
function encodeDramaId(id: string) {
  return `drama:${Buffer.from(id, 'utf8').toString('base64url')}`;
}

function decodeDramaId(id: string) {
  const encoded = /^drama:([A-Za-z0-9_-]+)$/u.exec(id)?.[1];
  if (encoded === undefined) throw new Error('Drama ID is invalid.');
  const decoded = Buffer.from(encoded, 'base64url').toString('utf8');
  if (decoded === '' || decoded.length > 256 || /[\u0000-\u001f]/u.test(decoded)) throw new Error('Drama ID is invalid.');
  return decoded;
}

function encodeEpisodeId(dramaId: string, sequence: string) {
  return `episode:${Buffer.from(JSON.stringify({ dramaId, sequence }), 'utf8').toString('base64url')}`;
}

function decodeEpisodeId(id: string): { readonly dramaId: string; readonly sequence: string } {
  const encoded = /^episode:([A-Za-z0-9_-]+)$/u.exec(id)?.[1];
  if (encoded === undefined) throw new Error('Episode ID is invalid.');
  try {
...
function encodeTarget(target: DiscoverTarget) {
  return `target:${Buffer.from(JSON.stringify(target), 'utf8').toString('base64url')}`;
}

function decodeTarget(value: string): DiscoverTarget {
  const encoded = /^target:([A-Za-z0-9_-]+)$/u.exec(value)?.[1];
  if (encoded === undefined) throw new Error('Discovery target is invalid.');
  try {
    const target: unknown = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8'));
...
function encodeCursor(scope: string, page: number) {
  return `page:${page}:${digest(scope).slice(0, 20)}`;
}

function cursorPage(cursor: string | null, scope: string) {
...
function mediaKind(value: string): 'hls' | 'video' {
...
function setText(target: Json, key: string, value: string | undefined) {
