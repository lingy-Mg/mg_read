/** Deterministic fixtures cover the full manga chain, origin-independent redirects, and HTTPS image proxy boundary. */
...
test('fixtures cover categories search detail redirected catalog ordered pages and Referer proxy', async () => {
  ...
  const resourceRequest = resources.find((request) => request.url.includes('/scomic/fixture/1.jpg')); assert.ok(resourceRequest); assert.equal(resourceRequest.kind, 'image'); assert.equal(new URL(resourceRequest.url).hostname, 'images.example'); assert.equal(resourceRequest.headers.Referer, 'https://mirror.example/comic/chapter/fixture-comic_real/0_0.html'); assert.match(resourceRequest.headers.Accept, /^image\//u);
  assert.ok(calls.length > 0); assert.ok(calls.every(({ url }) => url.hostname === 'cn.bzmgcn.com'));
