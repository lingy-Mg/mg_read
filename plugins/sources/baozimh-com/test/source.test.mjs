...
test('fixtures cover categories search detail redirected catalog ordered pages and Referer proxy', async () => {
  ...
  const resourceRequest = resources.find((request) => request.url.includes('s1.bzcdn.net')); assert.ok(resourceRequest); assert.equal(resourceRequest.kind, 'image'); assert.equal(resourceRequest.headers.Referer, 'https://www.baozimh.com/comic/chapter/fixture-comic_real/0_0.html'); assert.match(resourceRequest.headers.Accept, /^image\//u);
  assert.ok(calls.length > 0); assert.ok(calls.every(({ url }) => url.hostname === 'cn.bzmgcn.com'));
  assert.equal(calls.some(({ url }) => url.hostname === 's1.bzcdn.net'), false);
});
