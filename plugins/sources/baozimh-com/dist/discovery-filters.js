...
export const defaults = { type: 'all', region: 'all', state: 'all', filter: '*' };
export function pathFor(value) { return '/classify?' + new URLSearchParams(value).toString(); }
export function targetFor(value) { return 'filter:' + Buffer.from(JSON.stringify(value)).toString('base64url'); }
export function readFilters(target) {
    if (!/^filter:[A-Za-z0-9_-]{1,400}$/u.test(target))
    ...
    if (!value || dimensions.some(group => !group.values.some(([key]) => key === value[group.key])))
        throw new Error('Discovery target is invalid.');
    return { type: value.type, region: value.region, state: value.state, filter: value.filter };
}
export function filterSections(current) {
