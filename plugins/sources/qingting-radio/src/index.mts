...
export async function getChapters(request: { id: string }) {
  const id = contentId(request.id); const chapter = frozen({ id: `radio:${encodeKey(id)}:live`, title: '直播', order: 0, url: null, volumeTitle: '直播', wordCount: null, updatedAt: null, isLocked: null, attributes: [] });
