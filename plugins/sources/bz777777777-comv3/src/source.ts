/** Bz777 parser. Runtime browser provider owns Cookie, UA, challenge UI and session cache. */
...
interface BrowserRequest {
  ...
  readonly transport: 'webview';
  readonly timeoutMs: number;
  readonly maxResponseBytes: number;
}

interface BrowserResponse { readonly status: number; readonly body: string; }

export interface Context {
  readonly cacheDir: string;
  readonly browser: { readonly sessionV1: { request(request: BrowserRequest): Promise<BrowserResponse>; } };
