...
export class BzSource {
    ...
    parseList(html, base) {
        ...
        $('.book-all-list li.column-2,.column-list ul.list li,.bookbox,.result-item,.book-img-text li,.rank-list li,.list-group-item,ul.searchlist li').each((_, element) => {
            const root = $(element);
            const link = root.find('a.name,.name a,.right h4 a,.bookname a,h4 a,h3 a,a.book-name,.title a,a[href*="/book/"]').first();
            ...
            const coverRaw = image.attr('data-src') ?? image.attr('data-original') ?? image.attr('src');
            const authorText = clean(root.find('a.author,.author,dt,.s4').first().text());
            const author = clean(/作者[：:]?\s*(.+)$/u.exec(authorText ?? '')?.[1]) ?? authorText;
            const kind = clean(root.find('.cat,.s1').first().text());
            const latest = clean(root.find('p.update a,.update a,.s3 a').first().text());
    ...
    async getDetail(id) {
        const url = decode(id, 'book');
        const html = await this.#cache.getOrFetchText(url, detail, () => this.#browser(url, 'GET', null));
        const $ = cheerio.load(html);
        const title = clean($('meta[property="og:novel:book_name"]').attr('content')) ?? clean($('meta[property="og:title"]').attr('content')) ?? clean($('#info h1,h1.bookname,h1').first().text());
        if (title === null)
            throw new Error('Detail title is missing.');
        const info = clean($('#info,.bookinfo').first().text()) ?? '';
        const author = clean($('meta[property="og:novel:author"]').attr('content')) ?? clean(/作者[：:]?\s*([^\s<&]+)/u.exec(info)?.[1]);
        const coverRaw = clean($('meta[property="og:image"]').attr('content')) ?? clean($('#fmimg img,.bookimg img,.zhutu img').first().attr('src'));
        const intro = clean($('meta[property="og:description"]').attr('content')) ?? clean($('#intro,.intro,.bookinfo').first().text());
        const kind = clean($('meta[property="og:novel:category"]').attr('content'));
        const latest = clean($('meta[property="og:novel:latest_chapter_name"],meta[property="og:novel:lastest_chapter_name"]').first().attr('content'));
    ...
    async chapters(id) {
        const first = decode(id, 'book');
        const values = [];
        const seen = new Set();
        let url = first;
        for (let page = 0; page < 60; page += 1) {
            const html = await this.#cache.getOrFetchText(url, catalog, () => this.#browser(url, 'GET', null));
            const $ = cheerio.load(html);
            const links = $('#list dd a,#chapterlist a,.chapterlist a,.chapter-list a,.listmain dd a,#catalog a,.directoryArea a').toArray();
            for (const element of links) {
                ...
                values.push({ id: `chapter:${token(chapter)}`, title, order: values.length, url: chapter.toString(), volumeTitle: null, wordCount: null, updatedAt: null, isLocked: false, attributes: Object.freeze([]) });
            }
            const next = $('a').filter((_, element) => clean($(element).text()) === '下一页').first().attr('href');
            if (next === undefined)
                break;
            const candidate = new URL(next, url);
            if (candidate.origin !== origin || candidate.toString() === url.toString())
                break;
            url = candidate;
        }
        if (values.length === 0)
            throw new Error('Catalog is empty.');
        return Object.freeze({ items: Object.freeze(values) });
    }
    async content(id, chapterId) {
        decode(id, 'book');
        let url = decode(chapterId, 'chapter');
        const lines = [];
        const seen = new Set();
        for (let page = 0; page < 15 && !seen.has(url.toString()); page += 1) {
            seen.add(url.toString());
            const html = await this.#browser(url, 'GET', null);
            const $ = cheerio.load(html);
            const root = $('.page-content,#chaptercontent,#content,.read-content,#chapterText,.chapter-content').first();
            for (const paragraph of root.find('p').length > 0 ? root.find('p').toArray() : [root.get(0)].filter(Boolean)) {
                const line = clean($(paragraph).text());
                if (line !== null && !/(?:本章未完|加入书签|章节报错)/u.test(line))
                    lines.push(line);
            }
            const next = $('a').filter((_, element) => clean($(element).text()) === '下一页').first().attr('href');
            if (next === undefined)
        ...
        return Object.freeze({ chapterId, contentKind: 'novel', title: null, updatedAt: null, text, pages: Object.freeze([]) });
    }
    ...
    async #browserNow(url, method, body) {
        let stage = 'page_open';
        try {
    ...
    async #verify(page, url) {
        this.context.log.info('source_browser_navigate_started');
        await page.navigate(url.toString(), { timeoutMs: browserTimeoutMs });
        this.context.log.info('source_browser_navigate_completed');
...
function decode(value, prefix) { const match = new RegExp(`^${prefix}:([A-Za-z0-9_-]+)$`, 'u').exec(value); if (match?.[1] === undefined)
    throw new Error('Opaque ID is invalid.'); const url = new URL(Buffer.from(match[1], 'base64url').toString('utf8'), origin); if (prefix === 'book' ? !isBook(url) : !isChapter(url))
    throw new Error('Opaque ID is invalid.'); return url; }
function isBook(url) { return url.origin === origin && (/\/book\/\d+\/?$/u.test(url.pathname) || /\/\d+\/\d+\/?$/u.test(url.pathname)); }
function isChapter(url) { return url.origin === origin && (/\/book\/\d+\/\d+\.html$/u.test(url.pathname) || /\/(?:xs|look)\/\d+\/\d+\.html$/u.test(url.pathname)); }
...
function needsVerification(status, body) { return status === 403 || status === 503 || (typeof body === 'string' && isChallenge(body)); }
