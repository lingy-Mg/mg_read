 ...
 */
import { position, window as pageWindow } from './discovery-page.js';
...
export class ManhuaguiSource {
    ...
    #parseCards(html, pageUrl) {
        ...
        for (const item of captures(list, /<li\b[^>]*>([\s\S]*?)<\/li>/giu)) {
            ...
            const author = definition(item, '作\s*者');
            const category = definition(item, '类\s*别');
...
export function unpackPackedImageCode(html) {
...
function toDiscoveryItem(content) { return { content, rank: null, metric: null, recommendation: null }; }
function boundedPageSize(value) { if (!Number.isSafeInteger(value) || value <= 0)
    throw new Error('Page size is invalid.'); return Math.min(value, 50); }
function decodeCategory(target) { const id = target.startsWith('category:') ? target.slice(9) : ''; const value = categories.find((item) => item.id === id); if (value === undefined)
    throw new Error('Category target is invalid.'); return value; }
function decodeCursor(value) { if (value === null)
    return 1; if (!/^\d+$/u.test(value))
    throw new Error('Category cursor is invalid.'); const page = Number(value); if (!Number.isSafeInteger(page) || page < 1 || page > 10_000)
    throw new Error('Category cursor is invalid.'); return page; }
function categoryUrl(path, page) { if (page === 1)
    return new URL(path, mobileOrigin); return new URL(`${path.endsWith('/') ? path : `${path}/`}index_p${page}.html`, mobileOrigin); }
function findNextPage(html, path, page) { return links(html).some((link) => new URL(link.href, mobileOrigin).pathname === categoryUrl(path, page + 1).pathname); }
function categoryIcon(id) { return id === 'rank' ? 'ranking' : id === 'completed' ? 'completed' : id === 'ongoing' ? 'ongoing' : id === 'update' ? 'newRelease' : 'manga'; }
function definition(input, label) { return textFromMatch(input, new RegExp(`<dt>\\s*${label}\\s*[：:]?\\s*<\\/dt>\\s*<dd>([\\s\\S]*?)<\\/dd>`, 'iu')); }
function parseStatus(value) { if (value === null)
    return 'unknown'; if (/完结|完本/iu.test(value))
    return 'completed'; if (/连载/iu.test(value))
    return 'ongoing'; if (/停更|暂停/iu.test(value))
    return 'hiatus'; return 'unknown'; }
function titleWithoutSuffix(value) { return value === null ? null : text(value.split('_')[0] ?? value); }
function encodeBookId(key) { return `manga:${key.book}`; }
function decodeBookId(id) { const match = /^manga:(\d+)$/u.exec(id); if (match?.[1] === undefined)
    throw new Error('Content ID is invalid.'); return { book: match[1] }; }
function bookKeyFromUrl(url) { const match = /^\/comic\/(\d+)\/?$/u.exec(url.pathname); if (match?.[1] === undefined)
    throw new Error('Source manga URL is invalid.'); return { book: match[1] }; }
function encodeChapterId(key) { return `chapter:${key.book}:${key.chapter}`; }
function decodeChapterId(id) { const match = /^chapter:(\d+):(\d+)$/u.exec(id); if (match?.[1] === undefined || match[2] === undefined)
    throw new Error('Chapter ID is invalid.'); return { book: match[1], chapter: match[2] }; }
function chapterIdFromUrl(url) { return encodeChapterId(chapterKeyFromUrl(url)); }
function chapterKeyFromUrl(url) { const match = /^\/comic\/(\d+)\/(\d+)\.html$/u.exec(url.pathname); if (match?.[1] === undefined || match[2] === undefined)
    throw new Error('Source chapter URL is invalid.'); return { book: match[1], chapter: match[2] }; }
function bookUrl(key) { return new URL(`/comic/${key.book}/`, desktopOrigin); }
function chapterUrl(key) { return new URL(`/comic/${key.book}/${key.chapter}.html`, desktopOrigin); }
function allowedPage(url) { return url.protocol === 'https:' && (url.origin === desktopOrigin || url.origin === mobileOrigin); }
function allowedReferer(url) { return allowedPage(url) && (/^\/comic\/\d+(?:\/\d+\.html)?\/?$/u.test(url.pathname) || url.origin === mobileOrigin); }
function allowedImage(url) { return url.protocol === 'https:' && (url.hostname === 'cf.mhgui.com' || url.hostname === 'i.hamreus.com') && url.pathname.length > 1; }
function normalizePageUrl(value, base) { if (value === undefined || value === '')
    return null; const url = new URL(value, base); return allowedPage(url) ? new URL(url.pathname + url.search, desktopOrigin) : null; }
function normalizeImageUrl(value, base) { if (value === undefined || value === '')
    return null; const url = value.startsWith('//') ? new URL(`https:${value}`) : new URL(value, base); return allowedImage(url) ? url : null; }
function normalizeHost(value) { const normalized = value.startsWith('//') ? `https:${value}` : /^https?:\/\//iu.test(value) ? value : `https://${value}`; const url = new URL(normalized); if (!allowedImage(new URL('/placeholder.jpg', url)))
    throw new Error('Source image host is invalid.'); return url; }
function joinPath(path, file) { return `/${[path, file].map((value) => value.replace(/^\/+|\/+$/gu, '')).filter(Boolean).join('/')}`; }
function mimeType(url) { const extension = /\.([^.]+)$/u.exec(url.pathname)?.[1]?.toLowerCase(); return extension === 'jpg' || extension === 'jpeg' ? 'image/jpeg' : extension === 'png' ? 'image/png' : extension === 'webp' ? 'image/webp' : extension === 'gif' ? 'image/gif' : null; }
...
function skipComma(input, offset) { let index = offset; while (/[\s,]/u.test(input[index] ?? ''))
    index += 1; return index; }
function readNumber(input, offset) { const match = /^\d+/u.exec(input.slice(offset)); return match === null ? null : { value: Number(match[0]), end: offset + match[0].length }; }
...
function unpackCode(packed, radix, count, words) { const dictionary = new Map(); for (let index = count - 1; index >= 0; index -= 1) {
    const key = baseEncode(index, radix);
    const value = words[index];
    dictionary.set(key, value === undefined || value === '' ? key : value);
} return packed.replace(/\b\w+\b/gu, (word) => dictionary.get(word) ?? word); }
function baseEncode(value, radix) { const characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'; if (value === 0)
    return '0'; let output = ''; let remaining = value; while (remaining > 0) {
    output = characters[remaining % radix] + output;
    remaining = Math.floor(remaining / radix);
} return output; }
function extractJsonObject(code, marker) { const markerIndex = code.indexOf(marker); const start = code.indexOf('{', markerIndex + marker.length); if (markerIndex < 0 || start < 0)
    throw new Error('Source image object is missing.'); let depth = 0; let quote = ''; let escaped = false; for (let index = start; index < code.length; index += 1) {
...
function links(input) { return [...input.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/giu)].flatMap((match) => { const href = attribute(match[1] ?? '', 'href'); return href === undefined ? [] : [{ href, title: textFromHtml(match[2] ?? ''), tag: match[1] ?? '' }]; }); }
function firstLink(input, pattern) { return links(input).find((link) => pattern.test(link.href)); }
function captures(input, pattern) { return [...input.matchAll(pattern)].flatMap((match) => match[1] === undefined ? [] : [match[1]]); }
function firstCapture(input, pattern) { return input.match(pattern)?.[1] ?? null; }
function textFromMatch(input, pattern) { const value = firstCapture(input, pattern); return value === null ? null : textFromHtml(value); }
function textFromHtml(value) { return text(decodeHtml(value.replace(/<script\b[\s\S]*?<\/script>/giu, '').replace(/<style\b[\s\S]*?<\/style>/giu, '').replace(/<[^>]+>/gu, ' '))); }
function text(value) { const normalized = value.replace(/\u00a0/gu, ' ').replace(/\s+/gu, ' ').trim(); return normalized === '' ? null : normalized; }
function attribute(tag, name) { const escaped = name.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&'); const match = tag.match(new RegExp(`\\b${escaped}\\s*=\\s*(["'])([\\s\\S]*?)\\1`, 'iu')); return match?.[2] === undefined ? undefined : decodeHtml(match[2]); }
function decodeHtml(input) { const named = { amp: '&', apos: "'", gt: '>', lt: '<', nbsp: ' ', quot: '"' }; return input.replace(/&(#(?:x[0-9a-f]+|\d+)|[a-z]+);/giu, (entity, body) => { const value = body.toLowerCase(); if (value.startsWith('#x'))
    return String.fromCodePoint(Number.parseInt(value.slice(2), 16)); if (value.startsWith('#'))
    return String.fromCodePoint(Number.parseInt(value.slice(1), 10)); return named[value] ?? entity; }); }
function isRecord(value) { return typeof value === 'object' && value !== null && !Array.isArray(value); }
function stringValue(value) { return typeof value === 'string' && value !== '' ? value : typeof value === 'number' && Number.isFinite(value) ? String(value) : null; }
