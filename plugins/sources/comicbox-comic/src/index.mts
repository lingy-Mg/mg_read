...
function parseCards(source: string, selector: string): Item[] {
  const $ = load(source);
...
function chapterNodes($: ReturnType<typeof load>) {
  const result = new Map<string, { path: string; title: string }>();
...
function firstImage($: ReturnType<typeof load>, scope: string) {
...
function summary(value: Item) {
  return frozen({ id: `manga:${encode(value.path)}`, title: value.title, contentKind: 'manga' as const, coverOrientation: 'portrait' as const,
    author: null, url: new URL(value.path, base).toString(), coverUrl: imageProxy(value.cover, `${base}/`), description: value.description || null,
    language: 'zh-CN', status: 'unknown' as const, access: 'unknown' as const, wordCount: null, chapterCount: null, publishedAt: null,
    updatedAt: null, latestChapter: null, categories: [], tags: [], attributes: [] });
}

function imageProxy(value: string, referer: string) {
...
function chapterId(book: string, path: string) { return `manga:${encode(book)}:chapter:${encode(path)}`; }
function chapterPath(id: string, book: string) {
...
function clean(value: string) { return value.replaceAll(/\s+/gu, ' ').trim(); }
...
function requireContext() { if (!context) throw new Error('Source is not activated.'); return context; }
