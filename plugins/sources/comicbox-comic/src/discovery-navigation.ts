...
export type Filters = { tag: string; area: string; end: string };
export const defaultFilters: Filters = { tag: '-1', area: '-1', end: '-1' };
export function filterTarget(value: Filters) { return 'browse:' + Buffer.from(JSON.stringify(value)).toString('base64url'); }
export function readFilters(target: string): Filters {
  ...
  let value: unknown;
  try { value = JSON.parse(Buffer.from(target.slice(7), 'base64url').toString('utf8')); } catch { throw new Error('Discovery target is invalid.'); }
...
export function filterSections(html: string, current: Filters) {
  const $ = load(html);
  return $('.sp-filter-group').toArray().flatMap((element, index) => {
    ...
    const categories = group.find('dd[data-val]').toArray().flatMap(node => {
      ...
      const key = /active\(this,['"](tag|area|end)['"]\)/u.exec(item.attr('onclick') ?? '')?.[1] as keyof Filters | undefined;
      if (!key || !label || value === undefined || value.length > 80) return [];
      const target = filterTarget({ ...current, [key]: value });
      try { readFilters(target); } catch { return []; }
      return [{ id: target, title: label, target, count: null, url: null, icon: 'manga' }];
    });
    if (!title || categories.length === 0) return [];
    const children = [];
    for (let offset = 0; offset < categories.length; offset += 32) children.push({ type:'categoryCollection', id:'comicbox-filter-list-'+index+'-'+offset, layout:'chips', categories:categories.slice(offset,offset+32) });
    return [{ type: 'section', id: 'comicbox-filter-' + index, title, subtitle: null, icon: 'category', children }];
  });
}
export function hasNextPage(html: string, page: number) {
  const $ = load(html);
  return $('.pagination a[href]').toArray().some(node => {
    try { return Number(new URL($(node).attr('href')!, 'https://www.comicbox.xyz').searchParams.get('page')) === page + 1; } catch { return false; }
  });
}
