...
export async function getContent(request) {
    ...
    return frozen({
        chapterId: request.chapterId, contentKind: 'novel', title: null, updatedAt: null,
...
function summary(book) {
    ...
    return frozen({
        id: `midu:${book.id}`, title: book.title, contentKind: 'novel', coverOrientation: 'portrait',
        author: book.author || null, url: readerUrl(book.id), coverUrl: proxyImage(book.cover),
        description: book.description || null, language: 'zh-CN', status: book.status, access: 'unknown',
...
function parseLooseJson(raw) {
    ...
    try {
        ...
        for (let end = textValue.length; end > start; end -= 1) {
            ...
            catch { /* keep trimming a gateway suffix */ }
        }
        throw new Error('Source response is invalid.');
    }
}
...
function contentUrl(book, chapter) {
...
function stripHtml(value) { return value.replace(/<[^>]*>/gu, ' ').replaceAll('&nbsp;', ' '); }
function text(value) { return typeof value === 'string' || typeof value === 'number' ? String(value) : ''; }
function finiteNumber(value) { const result = Number(value); return Number.isFinite(result) && result >= 0 ? result : null; }
