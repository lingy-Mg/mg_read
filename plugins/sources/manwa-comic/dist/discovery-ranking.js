export const rankings = [['0', '人气榜'], ['1', '完结榜'], ['2', '新番榜']];
export async function rankingHtml(context, origin, sort) {
    if (!rankings.some(([id]) => id === sort))
    ...
    try {
        ...
        if (sort !== '0') {
            const changed = await page.executeJavaScript(`return new Promise(resolve=>{
        ...
        button.click();
            ...
            if (!changed)
                throw new Error('Ranking did not finish loading.');
        }
