...
export function readCursor(cursor, target) {
    const parts = cursor?.split('|') ?? [];
    if (parts.length > 2 || (parts[1] !== undefined && !/^\d{1,18}(?:,\d{1,18}){0,63}$/u.test(parts[1])))
        throw new Error('Discovery cursor is invalid.');
    return { ...position(parts[0] ?? null, target), seen: new Set(parts[1]?.split(',') ?? []) };
}
export function continueAt(target, page, offset, seen) {
