...
export function browsePath(target: string) {
  const raw = target.startsWith('browse:') ? target.slice(7) : '';
  if (!/^\/(?:cate(?:\/[a-z0-9-]+)?|rank|discover)\/?(?:\?[a-zA-Z0-9_=&%.-]+)?$/u.test(raw) || raw.length > 300) throw new Error('Discovery target is invalid.');
  return raw;
}
export function navigation(html: string) {
  ...
  const categories = $('a[href]').toArray().flatMap(node => {
    ...
    try { browsePath('browse:' + href); } catch { return []; }
    if (!title || seen.has(href) || seen.size >= 80) return []; seen.add(href);
    if (/^\/rank\/?$/u.test(href)) return rankings.map(([id,title])=>({id:'rank:'+id,title,target:'rank:'+id,count:null,url:null,icon:'ranking'}));
    return [{ id:'browse:'+href, title, target:'browse:'+href, count:null, url:null, icon:'manga' }];
  });
  const children = [];
  for (let offset = 0; offset < categories.length; offset += 32) children.push({type:'categoryCollection',id:'manwa-site-links-'+offset,layout:'chips',categories:categories.slice(offset,offset+32)});
  return categories.length ? [{type:'section',id:'manwa-site-navigation',title:'站点分类与排行',subtitle:null,children}] : [];
}
export function cards(html: string) {
  const $ = load(html), seen = new Set<string>();
  return $('a[href]').toArray().flatMap(node => {
    const link = $(node), id = /^\/comic\/(\d+)$/u.exec(link.attr('href') ?? '')?.[1], title = link.find('.title').text().trim();
    if (!id || !title || seen.has(id)) return []; seen.add(id);
