...
if (resolve(process.argv[1] ?? '') === fileURLToPath(import.meta.url)) {
  ...
  const artifact = await buildPluginArtifact(); const root = resolve(projectRoot, 'artifacts'); const target = resolve(root, artifact.fileName);
  await mkdir(root, { recursive: true }); await rm(target, { force: true }); await writeFile(target, artifact.bytes, { mode: 0o444 });
