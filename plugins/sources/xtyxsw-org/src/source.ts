...
export class TianyueSource{
  ...
  parseList(html:string,pageUrl:URL,fallbackCategory:string|null):readonly ContentSummary[]{
    if(/找不到您要搜索的内容/u.test(html))return Object.freeze([]);const $=cheerio.load(html);const seen=new Set<string>();const items:ContentSummary[]=[];
    const push=(url:URL,title:string,author:string|null,cover:string|null,description:string|null,latestTitle:string|null,latestUrl:URL|null,category:string|null)=>{const bookId=bookNumber(url);if(bookId===null||seen.has(bookId))return;seen.add(bookId);items.push(summary({bookId,title,author,url:readUrl(bookId),coverUrl:cover===null?null:this.#proxyImage(new URL(cover,pageUrl),pageUrl),description,status:'unknown',updatedAt:null,latestTitle,latestUrl,categories:category===null?[]:[category]}));};
    $('#alistbox').each((_,element)=>{const root=$(element);const titleLink=root.find('.info .title h2 a[href]').first();const link=titleLink.length>0?titleLink:root.find('.pic a[href]').first();const href=link.attr('href');const title=clean(link.text())??clean(root.find('.pic img').first().attr('alt'));if(href===undefined||title===null)return;const latest=root.find('.sys a[href]').first();const latestHref=latest.attr('href');push(new URL(href,pageUrl),title,clean(root.find('.info .title span a').first().text())??stripAuthor(clean(root.find('.info .title span').text())),root.find('.pic img').first().attr('src')??null,clean(root.find('.intro').text()),clean(latest.text()),latestHref===undefined?null:new URL(latestHref,pageUrl),fallbackCategory);});
    $('ul.list li').each((_,element)=>{const root=$(element);const link=root.find('p.bookname a[href]').first();const href=link.attr('href');const title=clean(link.text());if(href===undefined||title===null)return;const latest=root.find('p.data a[href]').filter((_,a)=>!$(a).hasClass('layui-btn')).first();const latestHref=latest.attr('href');push(new URL(href,pageUrl),title,clean(root.find('p.data a.layui-btn').first().text()),root.find('img').first().attr('src')??null,null,clean(latest.text()),latestHref===undefined?null:new URL(latestHref,pageUrl),fallbackCategory);});
