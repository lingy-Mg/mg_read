const base = 'https://hsck4.26img.com';
...
export async function getContent(request) {
    ...
    const parsed = parseDetail(await fetchDetail(id), id);
    if (parsed.mediaUrl === null || !safeMediaUrl(parsed.mediaUrl))
...
function parseList(html) {
    ...
    for (const match of html.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/giu)) {
        ...
        if (previous === undefined || (previous.coverUrl === null && candidate.coverUrl !== null))
...
function summary(id, title, cover, updatedAt, chapterCount) {
...
function paginate(values, limit, state, prefix, pages) {
    const end = Math.min(values.length, state.offset + limit);
    const items = values.slice(state.offset, end);
    let nextCursor = null;
    if (end < values.length)
        nextCursor = prefix + ':' + state.page + ':' + end;
    else if (state.page < pages)
        nextCursor = prefix + ':' + (state.page + 1) + ':0';
    return frozen({ items, nextCursor });
}
function parseCursor(cursor, prefix) {
    if (cursor === null)
        return frozen({ page: 1, offset: 0 });
...
function categoryUrl(code, page) {
