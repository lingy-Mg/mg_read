...
这不替代 Android 的正式 `.mgplugin` 验收。发现分类列表在一小时内直接命中，超过一小时先显示旧的
...
缓存实现来自仓库本地 `@mgread/plugin-html-cache`，不是 npm 发布包。构建时自动同步到本书源的
`packages/`，打包器会随 `.mgplugin` 收入它；Runtime 再根据 `package-lock.json` 将其恢复成普通
`node_modules` 依赖，无需安装脚本或网络下载。
