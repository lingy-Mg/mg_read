...
export class ShukugeSource {
    ...
    async getContent(id, chapterId) {
        ...
        for (let pageIndex = 0; pageIndex < 10 && !visited.has(url.toString()); pageIndex += 1) {
            ...
            if (nextHref === undefined || nextHref.startsWith('javascript:'))
                break;
            const next = new URL(nextHref, url);
            if (!isChapterPageContinuation(next, bookUrl, url))
                break;
            url = next;
        }
        return Object.freeze({
...
function encodeBookId(url) { return `book:${token(normalizeBookUrl(url).pathname)}`; }
...
function normalizeBookUrl(url) { return new URL(url.pathname.replace(/\/?$/u, '/'), origin); }
function isBookUrl(url) { return url.origin === origin && /^\/book\/\d+\/?$/u.test(url.pathname); }
function isCatalogUrl(url, bookUrl) { return url.origin === origin && url.pathname.startsWith(normalizeBookUrl(bookUrl).pathname) && /(?:index|index_\d+)\.html$/u.test(url.pathname); }
function isChapterUrl(url, bookUrl) { return url.origin === origin && url.pathname.startsWith(normalizeBookUrl(bookUrl).pathname) && /^\/book\/\d+\/\d+\.html$/u.test(url.pathname); }
function isChapterPageContinuation(next, bookUrl, current) {
    if (next.origin !== origin || !next.pathname.startsWith(normalizeBookUrl(bookUrl).pathname))
        return false;
...
function stripLabel(value, label) { return value === null ? null : clean(value.replace(new RegExp(`^${label}[：:]?\\s*`, 'u'), '')); }
