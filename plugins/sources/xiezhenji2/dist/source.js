...
export class Xiezhenji2Source {
    ...
    parseList(html, pageUrl) {
        ...
        $('#post-list .post-item, main .post-item, .post-item').each((_, element) => {
            const root = $(element);
            const link = root.find('h5.post-title a[href], a.plain[href], a[href]').first();
            const href = link.attr('href');
            const title = text(link.text()) ?? text(root.find('a[aria-label]').attr('aria-label'));
    ...
    async #cachedHtml(url, policy) { return this.#cache.getOrFetchText(url, policy, () => this.#html(url)); }
    async #html(url) {
    ...
    #proxyImage(url, referer) { if (url.protocol !== 'https:' || url.hostname !== 'cosplaytele.com' || referer.origin !== origin)
...
function isPostUrl(url) { return url.origin === origin && /^\/[^/?#]+\/?$/u.test(url.pathname) && !/^\/(?:category|tag|author|page|wp-json|wp-content)\//u.test(url.pathname); }
function text(value) { const result = value?.replace(/\s+/gu, ' ').trim() ?? ''; return result === '' ? null : result; }
...
function firstListingImageUrl(values, base) { for (const value of values) {
    if (value === undefined)
        continue;
    try {
    ...
    catch { /* Ignore malformed and placeholder candidates. */ }
} return null; }
function srcsetUrls(value) { return value === undefined ? Object.freeze([]) : Object.freeze(value.split(',').map((candidate) => candidate.trim().split(/\s+/u)[0]).filter((candidate) => candidate !== undefined && candidate !== '')); }
function parseCount(value) { const match = /(\d+)\s+photos?/iu.exec(value); const count = Number(match?.[1]); return Number.isSafeInteger(count) && count >= 0 ? count : null; }
function collectPageUrls(html, base) { const $ = cheerio.load(html); return uniqueUrls($('.entry-content a[href], .page-links a[href], .nav-links a[href]').toArray().flatMap((element) => { const href = $(element).attr('href'); if (href === undefined)
    return []; const url = new URL(href, base); return url.origin === origin && /\/(?:page\/)?\d+\/?$/u.test(url.pathname) ? [url] : []; })); }
function parseImages(html, base) { const $ = cheerio.load(html); const links = $('.entry-content .gallery a[href]').toArray().flatMap((element) => { const href = $(element).attr('href'); return href === undefined ? [] : [new URL(href, base)]; }); const images = links.length === 0 ? $('.entry-content .gallery img, .entry-content img.attachment-full').toArray().flatMap((element) => { const raw = $(element).attr('data-src') ?? $(element).attr('data-lazy-src') ?? $(element).attr('src'); return raw === undefined ? [] : [new URL(raw, base)]; }) : links; return uniqueUrls(images.filter((url) => url.hostname === 'cosplaytele.com' && /\/wp-content\/uploads\//u.test(url.pathname) && /\.(?:jpe?g|png|webp|gif)$/iu.test(url.pathname) && !url.pathname.includes('cropped-icon-'))); }
function imageMime(url) { const extension = /\.([A-Za-z0-9]+)$/u.exec(url.pathname)?.[1]?.toLowerCase(); return extension === 'jpg' || extension === 'jpeg' ? 'image/jpeg' : extension === 'png' ? 'image/png' : extension === 'webp' ? 'image/webp' : extension === 'gif' ? 'image/gif' : null; }
