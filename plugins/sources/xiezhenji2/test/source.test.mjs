...
test('fixture chain covers discovery, search, detail, complete gallery and resource proxy', async (t) => {
  ...
  const listingCover = proxied.find((request) => new URL(request.url).pathname.endsWith('/fixture-cover.webp')); assert.ok(listingCover); assert.match(listingCover.headers['User-Agent'], /^Mozilla\/5\.0/u);
