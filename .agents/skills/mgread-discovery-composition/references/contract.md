...
只在修改公开组件形状、布局枚举、图标、Runtime 校验或 Flutter Facade 时读取本文件。

## 所有权边界

- 书源拥有真实数据、稳定 `id/target/cursor`、组件树和内容语义。
...
语义图标类型为 `PluginDiscoveryIcon`/`DiscoveryIcon`。tab、section、category 可选声明 `icon`；Runtime 接受缺省或 null 并归一化为 null，非白名单字符串必须失败。图标的真实 Material 映射只在 `lib/features/discovery/presentation/discovery_semantic_icons.dart`。
...
- 主应用渲染：`lib/features/discovery/presentation/runtime_discovery_page.dart` 与 `discovery_composite_components.dart`
- 插件公开模板：`templates/mg_read_plugin_template/src/mgread-api.ts`
- 真实书源编译期类型：各书源 `src/mgread-api.ts`
