...
final class LanSyncImportPreview {
  ...
  Set<String> get recommendedPluginIds => <String>{
    for (final entry in pluginPlans.entries)
      if (entry.value == LanSyncPluginPlanState.missing ||
          entry.value == LanSyncPluginPlanState.upgrade)
        entry.key,
  };
}
...
final class LanSyncReadingProgress {
  ...
  factory LanSyncReadingProgress.fromJson(Map<String, Object?> json) {
    final updatedAt = DateTime.tryParse(
      _requiredString(json, 'updatedAtUtc', maxLength: 64),
    );
    ...
    return LanSyncReadingProgress(
      ...
      totalReadingSeconds: _requiredInt(
        json,
        'totalReadingSeconds',
        min: 0,
        max: 1576800000,
...
final class LanSyncShelfItem {
  ...
  factory LanSyncShelfItem.fromJson(Map<String, Object?> json) {
    ...
    return LanSyncShelfItem(
      ...
      remoteContentId: _requiredString(
        json,
        'remoteContentId',
        maxLength: 2048,
      ),
      ...
      progress: rawProgress == null
          ? null
          : LanSyncReadingProgress.fromJson(_requiredMap(rawProgress)),
    );
  }
}
...
final class LanSyncManifest {
  const LanSyncManifest({
    required this.plugins,
    required this.shelfItems,
    required this.skippedShelfItems,
  });
  ...
  factory LanSyncManifest.fromJson(Map<String, Object?> json) {
    ...
    final plugins = <LanSyncPluginDescriptor>[
      for (final raw in rawPlugins)
        LanSyncPluginDescriptor.fromJson(_requiredMap(raw)),
    ];
    ...
    return LanSyncManifest(
      ...
      shelfItems: List.unmodifiable(<LanSyncShelfItem>[
        for (final raw in rawItems)
          LanSyncShelfItem.fromJson(_requiredMap(raw)),
      ]),
      skippedShelfItems: _requiredInt(
        json,
        'skippedShelfItems',
        min: 0,
        max: 100,
