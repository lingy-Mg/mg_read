...
final class DataSourceSystemErrorDialogHost extends StatefulWidget {
  const DataSourceSystemErrorDialogHost({required this.reporter, required this.child, this.copyReport = _copyToClipboard, super.key});

  final DataSourceSystemErrorReporter reporter;
  final Widget child;
  final DataSourceDiagnosticCopy copyReport;

  static Future<void> _copyToClipboard(String payload) => Clipboard.setData(ClipboardData(text: payload));

  @override
  State<DataSourceSystemErrorDialogHost> createState() => _DataSourceSystemErrorDialogHostState();
}

final class _DataSourceSystemErrorDialogHostState extends State<DataSourceSystemErrorDialogHost> {
    ...
    try {
      await showDialog<void>(
        ...
        builder: (BuildContext context) =>
            _DataSourceSystemErrorDialog(report: report, onCopy: () => widget.copyReport(report.copyPayload)),
      );
    } catch (_) {
...
final class _DataSourceSystemErrorDialog extends StatelessWidget {
  const _DataSourceSystemErrorDialog({required this.report, required this.onCopy});
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: SelectionArea(
        child: Text(
          '${report.isDevelopmentReload
              ? '开发数据源构建失败，当前运行版本保持不变。'
              : report.quarantinedCount > 0
              ? '发现无法加载的数据源，已自动隔离。其他数据源可以继续使用。'
              : '数据源加载失败，其他数据源可以继续使用。'}\n\n'
          '${report.dialogPayload}',
          style: theme.textTheme.bodyMedium?.copyWith(color: tokens?.mutedText),
        ),
      ),
      actions: <Widget>[
        TextButton(key: const Key('data-source-error-copy'), onPressed: () => unawaited(_copySafely()), child: const Text('复制诊断信息')),
        FilledButton(key: const Key('data-source-error-close'), onPressed: () => Navigator.of(context).pop(), child: const Text('关闭')),
      ],
    );
  }
