...
final class NovelReaderSession {
  ...
  Future<CatalogEntry?> itemAtIndex(int index) {
    ...
    return _library._catalog._findBounded(itemId: item.id, upperBound: catalogCount, position: index);
  }
  ...
  Future<Map<String, CatalogEntry>> itemsByRemoteIdentities(Iterable<String> remoteIdentities) {
    ...
    return _library._catalog._findManyBounded(itemId: item.id, upperBound: catalogCount, remoteIdentities: identities);
  }

  Future<Page<CatalogEntry>> page({String? after, int limit = 100}) {
    return _library._catalog._pageBounded(itemId: item.id, upperBound: catalogCount, after: after, limit: limit);
  }
  ...
  Future<void> cacheChapter({required CatalogEntry entry, required String text}) =>
      _library._content._cacheNovelChapterForEntry(item: item, entry: entry, text: text);

  /// Replaces one body using the entry version captured before the remote read.
  Future<void> refreshChapter({required CatalogEntry entry, required String text}) =>
      _library._content._refreshNovelChapterForEntry(item: item, entry: entry, text: text);
