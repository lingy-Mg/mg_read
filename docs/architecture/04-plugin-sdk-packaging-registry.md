...
不希望发布到 npm 的纯 JS/ESM 复用库，仍使用普通 `file:./packages/<package>` 依赖，不能从插件
目录引用仓库外路径。仓库可保留一个规范源副本，并在插件构建前同步为该插件 `packages/` 下的发布副本；
该副本、`package.json` 和 v3 lockfile 必须一并提交。`mgread pack` 自动把 `packages/` 收入
`.mgplugin`，Runtime 再依据 lockfile 把它物化至普通 `node_modules`。因此已安装插件不执行 npm、
不运行 install script、也不依赖发布时或安装时网络。
...
| 数据 | 默认规则 | 原因 |
| --- | --- | --- |
| 发现/分类/搜索列表 | 默认缓存，10 分钟刷新窗；低波动发现可明确采用一小时 stale-while-revalidate | 高频重复访问，结果仅为来源展示投影 |
...
超过刷新窗默认先在线刷新，只有网络/超时失败时才能把仍在上限内的旧条目作为离线回退。仅低波动
发现投影可明确选择 stale-while-revalidate：先返回过期投影、后台同键 single-flight 刷新、下次调用
使用新值。要求严格时效的详情/目录必须禁用过期回退，刷新失败应如实失败。HTTP 非成功响应、超限内容、
格式不符或损坏内容不得写入；缓存读写、淘汰或损坏处理失败一律降级为缓存未命中，不得改变书源业务结果。
