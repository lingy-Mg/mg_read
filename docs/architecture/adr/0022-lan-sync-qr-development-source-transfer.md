...
1. 发送端把协议版本、会话 token、端口和经过筛选的全部私有 IPv4 候选地址编码为
   `mgread://lan-sync/v2` 二维码。候选地址只接受 RFC1918 网段，并排除名称明显属于 WSL、
   Hyper-V、Docker、虚拟机、VPN 或隧道的网卡；不把回环、链路本地或 CGNAT 地址放入二维码。
   接收端并发进行有界 TCP/协议握手，采用首个完成 MgRead 握手的地址并关闭其余成功连接。
   二维码不包含六位确认码；扫码后双方仍必须显示并人工核对同一个六位码。
...
- 手机可以从二维码中的多个候选地址自动选择实际可达地址，并继续依赖六位确认码防止扫错会话。
