...
> **历史验收设计（默认不读）**：本套件对应已被 ADR-0011/ADR-0100 取代的 Runtime 业务
> Store。其崩溃、配额、canary 和平台证据思想仍可作为新测试的参考，但用例所有者、数据根和
> 业务权威结论均不再有效。当前主应用持久化验收见
> [11 主应用持久化独立验收](11-app-persistence-acceptance.md)。
