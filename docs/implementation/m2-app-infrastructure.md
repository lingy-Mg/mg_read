...
| `path_provider` | `2.1.6` | Flutter 官方 Application Support 目录插件；仅由 app 组合根定位主应用 persistence 数据根。 | BSD-3-Clause；支持 Android、Windows、macOS，会引入各平台插件注册。实际移动/macOS 包体待平台验收。 |
...
- `main()` 先 await 全局 settings initialize，再通过 `ProviderScope` override 显式注入；
  loading/ready/degraded/failed/closing/closed 可观察。该 Provider 不得被用作 Runtime 生命周期、
  `HostPort` 或服务注入容器。
...
- `core/settings` 是纯内存读取与异步 persistence 门面，不属于 M2.4 设置页面；本设置核心提交
  不修改深色模式 UI 或其测试，`themeMode` 的消费由独立 UI 交付维护，也不实现 reader/download
  设置。
