...
- 已配对会话使用 HTTP 业务接口和逐设备密钥 HMAC 请求认证；局域网传输不启用 TLS，所有局域网 HTTP 客户端必须强制
  `DIRECT`，不得继承应用的系统或自定义上游代理。广播只包含设备 ID、标签和本次端口，IP 取收到的数据包且不得持久化。应用首个请求与
  ...
  唤醒，再由桌面端反向建立认证 HTTP 会话；反向会话交换拉取/推送方向，按钮语义始终以发起端为准。
...
- Android 只有系统确认已连接 Wi-Fi 时才允许同步、唤醒和在线广播；无 Wi-Fi 时保持低频状态探测，不发送 UDP。
  ...
  开发书源变化合并四十五秒后再单向强制推送，桌面端开发变化可立即推动。显式拉取同样只允许发送端到接收端，
  已有书架、阅读进度或插件均由发送端覆盖；对端会话内部失败必须在关闭加密连接前回报
  原始阶段、稳定错误码和有界原因，HTTP 断连不得覆盖正在执行的真实阶段。
- 每台设备独立控制自动同步、方向以及插件/书架范围；解除配对同时删除本机 metadata 与共享密钥。未配对的
  临时传输仍要求发送端保持页面并逐次核对确认码。
- App 包传输与书架/进度/插件数据同步是两个独立入口和独立任务，只能由接收端用户确认后拉取；在线已配对设备
  公布目标平台、版本名和构建号，较新版本显示升级提示，同版本或较旧版本只能经显式“强制安装”触发。正常情况
  仅允许同平台传输；Windows Debug 可额外公布项目固定输出
  `build/app/outputs/flutter-apk/app-release.apk`，其版本必须读取对应 Android 构建 metadata，不能伪装成当前版本。
- App 包沿局域网直连 HTTP 传输并校验大小与 SHA-256。Android 交给系统安装器继续做用户确认、来源授权和签名
  校验；Windows 由独立更新进程在主进程退出后替换已声明的应用文件，禁止删除用户文件或 manifest 外的未知文件。
- 同步只包含书架身份/展示元信息、阅读进度和插件；书架任务不得包含目录、正文、封面字节、缓存、设置、书签、
  删除操作或内部路径，接收端后续使用对应数据源插件自行拉取内容。书架元信息与阅读进度是两个独立控制任务，
  每个选中的插件制品是一个独立传输任务；插件制品准备、上传和下载最多三个任务并发。Android Runtime 每次最多
  校验并激活三十二个插件，更多插件按有界子批次依次完成并汇总为一次同步结果；失败时必须取消当前未完成子批次。
  推送/拉取是单向操作，接收端已有对应数据时按发送端内容覆盖，不做双端合并。
- 活动开发书源以成功激活内容的 SHA-256 指纹和本机单调修订号判新旧；相同指纹不重复传输。强制推送/拉取时，
  发送端插件可覆盖接收端的普通安装、旧开发副本或不同的开发构建；构建失败继续保留旧代。成功激活开发代时从该
...
- 协议、JSON manifest、artifact、批次和会话均须版本化且有界；配对 HTTP v4 manifest 使用独立 `bookshelf`、
  `progress` 和 `plugins` 任务字段，插件制品使用独立 HTTP PUT/GET，下载遵循
  Range、ETag 和 If-Range，正确处理 200、206、416 与版本变化。超限、截断、未知版本和顺序错误必须稳定失败，
  禁止回退旧 TCP 帧协议。
...
- 主程序 `--source-check` 的视频实际检查必须使用生产 Runtime 播放资源、视频代理与 MediaKit 表面；只有取得首帧、
  进入播放并观察到进度前进才通过。HTTP/HLS 前缀探测属于前置资源证据，不能替代 `playback.video`。
