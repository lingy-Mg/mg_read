...
void main() {
  ...
  test('initializes with one registered batch and get stays store-free', () async {
  ...
  test('close wins an in-flight factory initialization and closes its store', () async {
    await manager.close();
    ...
    manager = AppSettingsManager(
      ...
      registry: settingsTestRegistry,
      policy: const SettingsPersistencePolicy(closeTimeout: Duration(milliseconds: 100)),
    );
  ...
  test('set is immediate, notifications are synchronous, writes coalesce', () async {
    await manager.initialize();
    var notifications = 0;
    ...
    expect(store.documents[appearanceDocument.kind]!.values[themeKey.id], 'dark');
  ...
  test('document groups use independent trailing debounce windows', () async {
    ...
    expect(store.writeBatches.first.map((document) => document.kind), [appearanceDocument.kind]);
  ...
  test('resetGroup removes registered keys but preserves unknown fields', () async {
    store.documents[appearanceDocument.kind] = SettingsDocument(
      id: appearanceDocument.id,
      kind: appearanceDocument.kind,
      values: {themeKey.id: 'dark', nullableLabelKey.id: null, 'futureExtension': 9},
      revision: 1,
    );
    await manager.initialize();
    ...
    expect(manager.get(nullableLabelKey), 'fallback');
    await manager.flush();
    expect(store.documents[appearanceDocument.kind]!.values, {'futureExtension': 9});
  });

  test('transaction publishes atomically and only changes touched groups', () async {
    await manager.initialize();
    ...
    await manager.flush();
    expect(store.writeBatches.last.map((document) => document.kind).toSet(), {appearanceDocument.kind, behaviorDocument.kind});
  });
  ...
  test('a mutation encodes only keys in the changed document group', () async {
    ...
    final registry = SettingsRegistry(keys: [appearanceKey, behaviorKey], documents: const [appearanceDocument, behaviorDocument]);
  ...
  test('collection settings cannot mutate memory through caller aliases', () async {
    ...
    final localManager = AppSettingsManager(
      store: localStore,
      registry: SettingsRegistry(keys: [collectionKey], documents: const [appearanceDocument]),
      policy: const SettingsPersistencePolicy(debounce: Duration(hours: 1)),
    );
    addTearDown(localManager.close);
    ...
    expect(() => (stored['nested'] as List<Object?>).add(3), throwsUnsupportedError);
    expect(localManager.status.documents[appearanceDocument.kind]!.generation, 1);
  });

  test('typed list settings retain their runtime type and immutability', () async {
    ...
    final localManager = AppSettingsManager(
      store: FakeSettingsStore(),
      registry: SettingsRegistry(keys: [listKey], documents: const [appearanceDocument]),
      policy: const SettingsPersistencePolicy(debounce: Duration(hours: 1)),
    );
    addTearDown(localManager.close);
  ...
  test('write failure keeps the session value dirty and retries', () async {
    ...
    final subscription = manager.statusChanges.listen((status) => observedStates.add(status.state));
    addTearDown(subscription.cancel);
    ...
    expect(manager.status.isDirty, isTrue);
    expect(manager.status.documents[appearanceDocument.kind]!.lastErrorCode, 'injected_write_failure');
  ...
  test('close is bounded when a persistence write never completes', () async {
    ...
    manager = AppSettingsManager(
      store: store,
      registry: settingsTestRegistry,
      policy: const SettingsPersistencePolicy(debounce: Duration(hours: 1), closeTimeout: Duration(milliseconds: 30)),
    );
    await manager.initialize();
    await manager.set(themeKey, 'dark');
  ...
  test('close is bounded when a synchronous stream listener is paused', () async {
    await manager.close();
    store = FakeSettingsStore();
    manager = AppSettingsManager(
      store: store,
      registry: settingsTestRegistry,
      policy: const SettingsPersistencePolicy(closeTimeout: Duration(milliseconds: 30)),
    );
    await manager.initialize();
    final subscription = manager.changes.listen((_) {});
    subscription.pause();
  ...
  test('flush bypasses a long debounce and persists the pending value', () async {
    await manager.close();
    store = FakeSettingsStore();
    manager = AppSettingsManager(
      store: store,
  ...
  test('future and corrupt groups are degraded, defaulted, and read-only', () async {
    store.documents[appearanceDocument.kind] = SettingsDocument(
      id: appearanceDocument.id,
      kind: appearanceDocument.kind,
      values: const {},
    ...
    expect(manager.get(themeKey), 'system');
    expect(manager.status.documents[appearanceDocument.kind]!.readOnly, isTrue);
    await expectLater(manager.set(themeKey, 'dark'), throwsA(isA<SettingsReadOnlyException>()));
    expect(store.writeCalls, 0);
  });

  test('CAS conflict reloads and merges the local patch over new fields', () async {
    store.documents[appearanceDocument.kind] = SettingsDocument(
      id: appearanceDocument.id,
      kind: appearanceDocument.kind,
    ...
    await manager.initialize();
    await manager.set(themeKey, 'dark');
    store.externalWrite(appearanceDocument.kind, {themeKey.id: 'light', nullableLabelKey.id: 'external', 'unknown': 'new'});
  ...
  test('CAS reload corruption clears the patch and exposes safe defaults', () async {
    store.documents[appearanceDocument.kind] = SettingsDocument(
      id: appearanceDocument.id,
      kind: appearanceDocument.kind,
    ...
    expect(manager.status.documents[appearanceDocument.kind]!.lastErrorCode, 'corrupt_document');
    expect(manager.status.documents[appearanceDocument.kind]!.readOnly, isTrue);
    expect(store.writeCalls, 1);
  });

  test('registry rejects duplicate setting IDs', () {
    expect(() => SettingsRegistry.fromKeys(const [themeKey, themeKey]), throwsArgumentError);
    final unsafeCollectionKey = SettingKey<List<String>>(
      id: 'appearance.unsafeList',
      documentKind: appearanceDocument.kind,
      defaultValue: <String>[],
      codec: SettingCodec<List<String>>((value) => value, (value) => List<String>.of((value! as List).cast<String>())),
      validator: (_) {},
    );
    expect(() => SettingsRegistry(keys: [unsafeCollectionKey], documents: const [appearanceDocument]), throwsArgumentError);
  });

  test('manager rejects an unregistered key object that spoofs a stable ID', () async {
    await manager.initialize();
  ...
  test('oversized encoded values are rejected before changing memory', () async {
    ...
    final registry = SettingsRegistry(keys: [largeKey], documents: const [appearanceDocument]);
    final localManager = AppSettingsManager(store: FakeSettingsStore(), registry: registry);
    addTearDown(localManager.close);
    await localManager.initialize();
    final oversized = <String, Object?>{for (var index = 0; index < 10; index++) 'extension$index': List.filled(7000, 'x').join()};

    await expectLater(localManager.set(largeKey, oversized), throwsArgumentError);
    expect(localManager.get(largeKey), isEmpty);
    expect(localManager.status.isDirty, isFalse);
  });

  test('diagnostics cover initialize, mutation, and write', () async {
    const settingValue = 'SETTINGS-SECRET-CANARY';
    await manager.close();
    ...
    manager = AppSettingsManager(
      ...
      policy: const SettingsPersistencePolicy(debounce: Duration(hours: 1), closeTimeout: Duration(seconds: 1)),
    );

    await manager.initialize();
    await manager.set(nullableLabelKey, settingValue);
    final result = await manager.flush();
