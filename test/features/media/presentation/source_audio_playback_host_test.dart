...
void main() {
  testWidgets('remembers continue choice and restores from the mini player', (tester) async {
    ...
    await tester.ensureVisible(find.byKey(const Key('audio-back')));
    ...
    expect(find.ancestor(of: find.byKey(const Key('audio-background-exit-dialog')), matching: find.byType(BackdropFilter)), findsNothing);
    final exitDialog = tester.widget<AlertDialog>(find.byKey(const Key('audio-background-exit-dialog')));
    expect(exitDialog.backgroundColor, isNull);
    expect(exitDialog.shape, isNull);
    expect(exitDialog.titleTextStyle, isNull);
    expect(exitDialog.contentTextStyle, isNull);
    expect(tester.widget<ModalBarrier>(find.byKey(const Key('audio-background-exit-barrier'))).color, Colors.black54);
    expect(tester.widget<FilledButton>(find.byKey(const Key('audio-background-continue'))).style, isNull);
    ...
    final miniGlass = tester.widget<DecoratedBox>(find.byKey(const Key('source-audio-mini-glass')));
    final miniDecoration = miniGlass.decoration as BoxDecoration;
    final miniGradient = miniDecoration.gradient! as LinearGradient;
    final miniTokens = AppThemeTokens.of(tester.element(find.byKey(const Key('source-audio-mini-glass'))));
    expect(miniGradient.colors, <Color>[miniTokens.surface.withValues(alpha: 0.58), miniTokens.mutedSurface.withValues(alpha: 0.38)]);
    expect(tester.widget<Material>(find.byKey(const Key('source-audio-mini-player'))).color, Colors.transparent);
