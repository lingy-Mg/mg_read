...
void main() {
  ...
  testWidgets('add device remains enabled while pairing is active', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: AppTheme.light(),
        home: Scaffold(
          body: PairedDevicesSection(
            state: const DeviceSyncState(pairingPhase: DevicePairingPhase.showingOffer),
            supportsScanner: false,
            onBeginPairing: () {},
            onApprovePairing: () {},
            onRejectPairing: () {},
    ...
    expect(tester.widget<TextButton>(find.byKey(const Key('device-sync-add-device'))).onPressed, isNotNull);
  });
}
