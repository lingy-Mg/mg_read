...
void main() {
  test(
    'observes a pre-subscription startup failure from the typed snapshot',
    () async {
  ...
  test(
    'observes a Windows Node exit after successful initial warmup',
    () async {
      final kit = DiagnosticsTestkit();
      final reporter = AppFatalErrorReporter(kit.manager);
      final source = _FakeRuntimeFatalDiagnosticSource();
      ...
      source.add(
        const RuntimeDiagnostic(
          ...
          level: RuntimeDiagnosticLevel.error,
          message:
              'The desktop Runtime process exited unexpectedly (exit code 1).',
        ),
      );
  ...
  test(
    'does not promote ordinary source diagnostics to the fatal dialog',
    () async {
      final kit = DiagnosticsTestkit();
      final reporter = AppFatalErrorReporter(kit.manager);
      final source = _FakeRuntimeFatalDiagnosticSource();
...
final class _FakeRuntimeFatalDiagnosticSource
    implements RuntimeFatalDiagnosticSource {
  _FakeRuntimeFatalDiagnosticSource({
    this.latest = const <RuntimeDiagnostic>[],
  });

  final List<RuntimeDiagnostic> latest;
  final StreamController<RuntimeDiagnostic> _controller =
      StreamController<RuntimeDiagnostic>.broadcast(sync: true);
